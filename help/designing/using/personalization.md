...
* 이메일 발신자 개인화 다음을 참조하십시오 [보낸 사람 개인화](#personalizing-the-sender).
* 이메일 제목을 개인화합니다. 다음을 참조하십시오 [이메일 제목란 개인화](../../designing/using/subject-line.md#subject-line).
* 조건부 콘텐츠 만들기 다음을 참조하십시오 [이메일에서 동적 콘텐츠 정의](#defining-dynamic-content-in-an-email).

## 보낸 사람 개인화 {#personalizing-the-sender}

보낸 메시지의 헤더에 나타나는 보낸 사람의 이름을 정의하려면 **[!UICONTROL Properties]** 이메일 디자이너 홈 페이지의 탭(홈 아이콘을 통해 액세스 가능). 자세한 내용은 [이메일 발신자 정의](../../designing/using/subject-line.md#email-sender).
...
1. 외부 URL을 삽입하고 해당 매개 변수를 지정합니다. 다음을 참조하십시오 [링크 삽입](../../designing/using/links.md#inserting-a-link).
...
>
>콘텐츠 블록을 선택할 때 다음과 같은 요소를 선택할 수 없습니다. **미러 페이지 링크**. 이러한 유형의 블록은 링크 내에서 사용할 수 없습니다.
...
>[!NOTE]
>
>아래 이미지는 를 사용하여 개인화 필드를 삽입하는 방법을 보여 줍니다. [이메일 디자이너](../../designing/using/designing-content-in-adobe-campaign.md) 이메일.

콘텐츠에 개인화 필드를 추가하려면:

1. 텍스트 블록 내부를 클릭하고 **[!UICONTROL Personalize]** 아이콘을 클릭하고 다음을 선택합니다 **[!UICONTROL Insert personalization field]**. 이메일 디자이너 인터페이스에 대한 자세한 내용은 [이 섹션](../../designing/using/designing-content-in-adobe-campaign.md#email-designer-interface).
...
>[!NOTE]
>
>워크플로우에서 이메일을 만드는 경우 워크플로우에서 계산된 추가 데이터를 개인화 필드에서도 사용할 수 있습니다. 워크플로우에서 추가 데이터를 추가하는 방법에 대한 자세한 내용은 [데이터 강화](../../automating/using/about-targeting-activities.md#enriching-data) 섹션.
...
>[!NOTE]
>
>아래 이미지는 를 사용하여 콘텐츠 블록을 삽입하는 방법을 보여 줍니다. [이메일 디자이너](../../designing/using/designing-content-in-adobe-campaign.md) 이메일.

콘텐츠 블록을 추가하려면:

1. 텍스트 블록 내부를 클릭하고 **[!UICONTROL Personalize]** 아이콘을 클릭하고 다음을 선택합니다 **[!UICONTROL Insert content block]**. 이메일 디자이너 인터페이스에 대한 자세한 내용은 [이 섹션](../../designing/using/designing-content-in-adobe-campaign.md#email-designer-interface).
...
* **[!UICONTROL Database URL in emails (EmailUrlBase)]**: 이 콘텐츠 블록은 다음에서만 사용할 수 있습니다. **게재**.
* **[!UICONTROL Mirror page URL (MirrorPageUrl)]**: 이 콘텐츠 블록은 다음에서만 사용할 수 있습니다. **게재**.
* **[!UICONTROL Link to mirror page (MirrorPage)]**: 이 콘텐츠 블록은 다음에서만 사용할 수 있습니다. **게재**.
* **[!UICONTROL Greetings (Greetings)]**
* **[!UICONTROL Unsubscription link (UnsubscriptionLink)]**: 이 콘텐츠 블록은 다음에서만 사용할 수 있습니다. **게재**.
* **[!UICONTROL Social network sharing links (LandingPageViralLinks)]**: 이 콘텐츠 블록은 다음에서만 사용할 수 있습니다. **랜딩 페이지**.
* **[!UICONTROL Default sender name (DefaultSenderName)]**: 이 콘텐츠 블록은 다음에서만 사용할 수 있습니다. **게재**.
* **[!UICONTROL Name of default reply-to email address (DefaultReplyName)]**: 이 콘텐츠 블록은 다음에서만 사용할 수 있습니다. **게재**.
* **[!UICONTROL Email address of default sender (DefaultSenderAddress)]**: 이 콘텐츠 블록은 다음에서만 사용할 수 있습니다. **게재**.
* **[!UICONTROL Default error email address (DefaultErrorAddress)]**: 이 콘텐츠 블록은 다음에서만 사용할 수 있습니다. **게재**.
* **[!UICONTROL Default reply-to email address (DefaultReplyAddress)]**: 이 콘텐츠 블록은 다음에서만 사용할 수 있습니다. **게재**.
...
1. 클릭 **[!UICONTROL Resources > Content blocks]** 고급 메뉴에서 콘텐츠 블록 목록에 액세스합니다.
1. 다음을 클릭합니다. **[!UICONTROL Create]** 기존 콘텐츠 블록을 버튼하거나 복제합니다.
...
1. 다음을 선택할 수 있습니다. **[!UICONTROL Targeting dimension]**. 자세한 내용은 [타겟팅 차원 기본 정보](#about-targeting-dimension).

   ![](assets/content_bloc_04.png)

1. 다음을 선택할 수 있습니다. **[!UICONTROL Depends on format]** HTML 이메일과 텍스트 형식의 이메일에 대해 각각 하나씩, 두 개의 서로 다른 블록을 정의하는 옵션입니다. 그러면 편집기(HTML 및 텍스트)에 해당 콘텐츠를 정의할 수 있는 두 개의 탭이 표시됩니다.
...
>[!CAUTION]
>
>블록의 내용을 편집할 때 의 시작과 끝 사이에 추가 공백이 없는지 확인합니다. *if* 명령문입니다. HTML 시 공백은 화면에 표시되므로 컨텐츠 레이아웃에 영향을 미칩니다.
...
예를 들어 **[!UICONTROL Unsubscription link]** 블록의 타겟팅 차원은 입니다. **[!UICONTROL Profiles]** 여기에는 다음과 관련된 개인화 필드가 포함되어 있습니다. **[!UICONTROL Profiles]** 리소스. 따라서 **[!UICONTROL Unsubscription link]** 다음에서 차단 [이벤트 트랜잭션 메시지](../../channels/using/getting-started-with-transactional-msg.md#transactional-message-types): 해당 유형의 메시지 타겟팅 차원이 **[!UICONTROL Real-time events]**. 하지만 **구독 취소 링크** 의 블록 [프로필 트랜잭션 메시지](../../channels/using/getting-started-with-transactional-msg.md#transactional-message-types): 해당 유형의 메시지 타겟팅 차원이 **프로필**. 마지막으로 **[!UICONTROL Link to mirror page]** 블록에는 타겟팅 차원이 없으므로 모든 메시지에서 사용할 수 있습니다.
...
1. 이미지 속성 팔레트에서 **[!UICONTROL Enable personalization]** 옵션을 선택합니다.
   ...
   다음 **[!UICONTROL Source]** 필드가 표시되고 선택한 이미지가 다음으로 표시됩니다. **개인화됨** 를 입력합니다.

1. 다음 옆에 있는 연필을 클릭합니다. **[!UICONTROL Source]** 필드 단추 를 사용하십시오.
...
표현식 편집기는 [고급 표현식 편집](../../automating/using/editing-queries.md#about-query-editor) 섹션.

이러한 조건은 XTK 표현식 구문을 채택합니다(예: **context.profile.email !=&#39;&#39;** 또는 **context.profile.status=&#39;0&#39;**). 기본적으로 모든 필드가 표시됩니다.
...
다이내믹 콘텐츠 정의와 다름 [가시성 조건 정의](#defining-a-visibility-condition).

1. 조각, 구성 요소 또는 요소를 선택합니다. 이 예에서 이미지를 선택합니다.
1. 다음을 클릭합니다. **[!UICONTROL Dynamic content]** 아이콘을 클릭합니다.

   ![](assets/dynamic_content_2.png)

   다음 **[!UICONTROL Dynamic content]** 섹션은 왼쪽의 팔레트에 나타납니다.
...
1. 다음을 클릭합니다. **[!UICONTROL Edit]** 단추를 클릭하여 첫 번째 대체 변형의 표시 조건을 정의합니다.

   ![](assets/dynamic_content_4.png)

1. 레이블을 지정하고 조건으로 설정할 필드를 선택합니다. 예를 들어 **[!UICONTROL General]** 노드를 선택하고 **[!UICONTROL Age]** 필드
...
1. 모든 조건이 설정되면 조건이 적용될 우선 순위 순서를 정의하고 변경 사항을 저장합니다.
   ...
   콘텐츠는 위에서 아래로 우선 순위 순서대로 팔레트에 표시됩니다. 우선 순위에 대한 자세한 내용은 [이 섹션](#defining-dynamic-content-in-an-email).
...
1. 클릭 **[!UICONTROL Add a condition]** 새 콘텐츠 및 연결된 규칙을 추가합니다.
...
1. 을 사용하여 두 개의 서로 다른 동적 콘텐츠를 정의합니다 **두 가지 다른 조건**, 예를 들면 다음과 같습니다.
...
1. 따라서 동적 콘텐츠의 우선 순위를 정의해야 합니다. 우선 순위가 인 조건 **1** (따라서 해당 다이내믹 콘텐츠)는 우선 순위가 인 다른 조건이 있더라도 프로필에 전송됩니다 **2** 또는 **3** 이 프로필에서도 충족됩니다.
...
1. 에서 동적 콘텐츠 선택 **[!UICONTROL Properties]** 팔레트 및 클릭 **[!UICONTROL Edit]** 단추를 클릭합니다.

   ![](assets/delivery_content_48.png)

1. 레이블을 편집한 다음 **[!UICONTROL Age]** 의 필드 **[!UICONTROL Profile]** 노드.

   ![](assets/delivery_content_49.png)

1. 다음 항목 선택 **크거나 같음** 연산자가 입력한 후 **18** 을(를) 만들려면 **18세 이상** 표현식.

   ![](assets/delivery_content_50.png)

1. 새 항목 추가 **[!UICONTROL Age]** 조건.

   다음 항목 선택 **작거나 같음** 연산자 뒤에 값 필드에 27을 입력하여 **27세 미만** 표현식.

   ![](assets/delivery_content_51.png)

1. 변경 사항을 확인합니다.

**27세 이상 프로필을 타겟팅하려면 다음 작업을 수행하십시오.**

1. 팔레트에서 동적 콘텐츠를 선택하고 편집합니다.
1. 레이블을 편집한 다음 **[!UICONTROL Age]** 의 필드 **[!UICONTROL Profile]** 노드.
1. 추가 **보다 큼** 연산자 뒤에 값 필드에 27을 입력하여 **27세 이상** 표현식.
...
**18~27세 사이의 프로필을 타겟팅하려면 다음을 수행합니다.**

1. 원하는 구조 구성 요소를 선택하고 다이내믹 콘텐츠를 추가합니다.
1. 다이내믹 콘텐츠를 편집하고 타깃팅 표현식을 구성합니다. 을(를) 참조하십시오 [이미지 구성](#configuring-images).
1. 구조 구성 요소에서 원하는 위치에서 **[!UICONTROL Personalize]** 아이콘을 클릭하고 다음을 선택합니다 **[!UICONTROL Insert personalization field]**.

   ![](assets/delivery_content_53.png)

1. 표시되는 목록에서 다음을 선택합니다. **[!UICONTROL First name]** 필드 및 확인
...
1. 다이내믹 콘텐츠를 편집하고 타깃팅 표현식을 구성합니다. 을(를) 참조하십시오 [이미지 구성](#configuring-images).
1. 구조 구성 요소에서 원하는 위치에서 **[!UICONTROL Personalize]** 아이콘을 클릭하고 다음을 선택합니다 **[!UICONTROL Insert personalization field]**.
1. 선택 **[!UICONTROL Title]** 을 클릭합니다.
1. 유사하게 진행하여 다음을 추가하십시오. **[!UICONTROL Last name]** 필드.
...
미리 보기를 사용하면 를 보내기 전에 개인화 필드 및 동적 콘텐츠가 올바르게 구성되었는지 확인할 수 있습니다. **[!UICONTROL Proofs]**. 미리 보기 중에 이메일 타겟에 해당하는 다른 테스트 프로필을 선택할 수 있습니다.
