...
이러한 외부 계정은 Campaign 워크플로에서 데이터 액세스 및 관리에 사용할 수 있습니다.
...
* Microsoft Azure Blob 저장 공간. 이 작업에 대한 자세한 정보는 [이 섹션](#microsoft-azure-external-account)을 참조하십시오.
* OAuth 2.0. 이 작업에 대한 자세한 정보는 [이 섹션](#oauth-account)을 참조하십시오.

>[!NOTE]
>
>기타 유형의 외부 계정은 제품 프로비전 프로세스 동안 Adobe에서 사용됩니다. Campaign Standard 17.9 릴리스부터는 FTP 외부 계정을 정의할 수 있지만 새로운 워크플로 활동에서는 더 이상 사용할 수 없습니다. 이미 연결을 설정한 경우 여전히 활성화되어 있습니다.
...
외부 계정은 기술 워크플로 또는 캠페인 워크플로와 같은 기술 프로세스에서 사용됩니다. 워크플로 또는 다른 애플리케이션(Adobe Target, Experience Manager 등)과의 데이터 교환에서 파일 전송을 설정할 때는 외부 계정을 선택해야 합니다.

1. **[!UICONTROL Create]** 버튼을 클릭합니다.
1. 레이블을 입력합니다. 워크플로에서 외부 계정을 선택할 때 레이블과 ID가 사용됩니다.
1. 만들 계정 유형을 선택합니다.
...
외부 계정이 만들어지고 계정 목록에 추가됩니다. 이제 워크플로 활동 및 게재 속성에서 데이터/파일 전송 또는 라우팅 구성에 사용할 수 있습니다.
...
* **부여 형식**: **클라이언트 자격 증명**&#x200B;만 지원됩니다.
* **보안 API URL**: 인증 끝점을 입력하십시오.
* **OAuth 2.0 중요 자격 증명**: 이 섹션은 기본적으로 중요한 자격 증명을 위한 것입니다. 자격 증명 값은 추가된 후 화면에 마스킹됩니다. 이 시점에서 읽을 수 없거나 편집할 수 없습니다. 인증 끝점에 특정 자격 증명을 POST 본문 매개 변수 대신 HTTP 인증 헤더에 삽입해야 하는 경우, 해당 자격 증명에 대해 헤더에 포함 옵션을 선택할 수 있습니다.
* **OAuth 2.0 중요하지 않은 자격 증명**: 이 섹션은 기본적으로 중요하지 않은 자격 증명을 위한 것입니다. 자격 증명 값은 추가되면 화면에 표시되며, 편집할 수도 있습니다.  인증 끝점에 특정 자격 증명을 POST 본문 매개 변수 대신 HTTP 인증 헤더에 삽입해야 하는 경우, 해당 자격 증명에 대해 헤더에 포함 옵션을 선택할 수 있습니다.

계정 정보를 입력한 후 **연결 테스트**&#x200B;를 클릭하여 외부 계정이 올바르게 구성되었는지 확인하십시오.
...
Azure Blob 저장 공간 커넥터를 사용하여 **[!UICONTROL Transfer file]** 워크플로 활동을 통해 데이터를 Adobe Campaign으로 가져오거나 내보낼 수 있습니다. 자세한 정보는 이 [섹션](../../automating/using/transfer-file.md#azure-blob-configuration-wf)을 참조하십시오.
...
### Microsoft Azure Blob 스토리지 권장 사항 {#azure-blob-recommendations}
...
파일 전송 완료 후 워크플로 로그에서 성능 지표를 찾을 수 있습니다.
