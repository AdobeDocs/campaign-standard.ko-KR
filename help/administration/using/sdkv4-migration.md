---
title: Adobe Experience Platform SDK으로 SDK v4 모바일 애플리케이션 마이그레이션
description: 모바일 애플리케이션을 SDK v4에서 Adobe Experience Platform SDK으로 마이그레이션하는 방법에 대해 알아봅니다.
audience: channels
...
Adobe Experience Platform Mobile 버전 4 SDK에 대한 지원이 2021년 8월 31일부로 종료되었습니다. 이 레거시 버전의 SDK을 계속 사용하는 경우 2024년 6월 말 **전에 Adobe Experience Platform SDK**&#x200B;로 구현을 업데이트해야 합니다. 이 문서에서 Adobe Experience Platform SDK으로 마이그레이션하는 방법을 알아봅니다.

>[!IMPORTANT]
>
> SDK V4 모바일 애플리케이션을 Adobe Experience Platform SDK으로 마이그레이션하기 전에 문서를 주의 깊게 읽으십시오.

## SDK V4 마이그레이션 기본 정보

Adobe Campaign Standard은 SDK V4를 Adobe Experience Platform SDK을 사용하는 모바일 애플리케이션과 별도의 애플리케이션으로 사용하여 모바일 애플리케이션을 처리합니다.
...
>[!NOTE]
>
> 이 페이지에서는 SDK v4 모바일 애플리케이션을 새로 생성된 Adobe Experience Platform SDK 애플리케이션으로의 마이그레이션에 대해 설명합니다. SDK v4 모바일 응용 프로그램은 **[!UICONTROL Configured]** **[!UICONTROL Property status]**&#x200B;이(가) 있는 Adobe Experience Platform SDK 모바일 응용 프로그램과 병합되지 않습니다.
...
## 모바일 애플리케이션을 SDK v4에서 Adobe Experience Platform SDK으로 마이그레이션 {#how-to-migrate}
...
이 마이그레이션 후에는 모바일 애플리케이션의 V4 버전에서 수집한 구독자와 모바일 애플리케이션의 AEP 버전에서 수집한 새 구독자를 마이그레이션된 애플리케이션에서 사용할 수 있습니다.
...
### Q: SDK v4 모바일 애플리케이션에서 Adobe Experience Platform SDK으로 모바일 애플리케이션 마이그레이션 탭이 표시되지 않습니다. {#tab-not-visible}
...
### Q: Adobe Experience Platform SDK으로 모바일 애플리케이션 마이그레이션 탭에 데이터 없음 메시지가 나타납니다. {#no-data}
...
A: 마이그레이션 프로세스는 SDK v4 구독자 및 속성을 유지합니다. Adobe Experience Platform SDK 애플리케이션의 태그 관련 정보만 유지합니다. Adobe Experience Platform SDK 애플리케이션의 구독자 및 기타 데이터가 손실됩니다. 데이터 손실을 방지하기 위해 **[!UICONTROL Ready to Configure]** **[!UICONTROL Property Status]**&#x200B;이(가) 있는 Adobe Experience Platform SDK 애플리케이션만 마이그레이션할 수 있습니다.
...
### Q: 마이그레이션 후 새로 만든 Adobe Experience Platform SDK 애플리케이션을 어디에서 찾을 수 있습니까? {#aep-not-visible}

A: 마이그레이션에 사용되는 새로 만든 Adobe Experience Platform SDK 애플리케이션이 별도의 애플리케이션으로 존재하지 않습니다. 마이그레이션된 SDK v4 애플리케이션만 사용할 수 있습니다.

### Q: SDK v4 모바일 애플리케이션 조직 단위가 A(조직 단위 ALL의 하위)로 설정되어 있고 Adobe Experience Platform SDK이 ALL로 설정되어 있는 경우. 모바일 애플리케이션을 마이그레이션하려면 어떻게 해야 합니까? {#v4-org-unit}
...
A: SDK V4 모바일 애플리케이션을 만들 때 시스템이 이 정보를 저장하지 않습니다. SDK V4 모바일 애플리케이션을 Adobe Experience Platform SDK 모바일 애플리케이션으로 마이그레이션할 때 마이그레이션된 모바일 애플리케이션에는 이러한 종류의 정보도 포함되지 않습니다. 사용자가 새 iOS 인증서 또는 Android 키를 업로드하는 즉시 키 또는 인증서의 다른 세부 정보가 저장되고 **[!UICONTROL Push channel settings]** 드롭다운 아래에 올바르게 표시됩니다.
