...
1. **티켓 만들기** - 하위 도메인이 구성되면 Adobe이 프로덕션 환경에서 설정합니다. 이를 요청하려면, [Client Care에 대한 티켓 만들기](https://helpx.adobe.com/kr/enterprise/using/support-for-experience-cloud.html) 다음 정보가 포함된 경우:
...
1. **게재 템플릿 만들기** - 새 브랜드를 사용할 수 있게 되면 가장 좋은 방법은 이 새 브랜드를 참조하는 빈 게재 템플릿을 하나 이상 만드는 것입니다. [자세히 알아보기](#linking-a-brand-to-a-template)

1. **게재 기능 지침 확인** - 새 도메인 사용을 시작하기 전에 Adobe 전달성 팀과 전략에 대해 논의해야 합니다. 모범 사례를 정의하는 데 도움이 됩니다. 예를 들어 도메인 간에 IP를 분할하기 위해 새 선호도를 만들어야 하는지 및/또는 램프 업 계획을 정의해야 하는지 등이 여기에 포함됩니다. 게재 가능성 모범 사례에 대해 자세히 알아보기 [이 섹션에서](../../sending/using/about-deliverability.md).
...
1. Adobe Campaign 관리자는 먼저 **[!UICONTROL Administration > Instance settings > Brand configuration]** 메뉴 및 추가 **트로픽의 휴가** 요소를 만들고 **[!UICONTROL ID]** 및 **[!UICONTROL Header parameters of sent emails]** 브랜드.
