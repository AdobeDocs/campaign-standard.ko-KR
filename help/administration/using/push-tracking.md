...
Campaign Standard 추적을 구현하려면 모바일 앱에 Mobile SDK가 포함되어야 합니다. 이러한 SDK는 Adobe Mobile Services에서 사용할 수 있습니다.
...
노출 횟수 추적의 경우 trackAction() 함수를 호출할 때 동작에 대해 값 &quot;7&quot;을 전송해야 합니다.
...
클릭 추적의 경우 trackAction() 함수를 호출할 때 동작에 대해 값 &quot;2&quot;를 보내야 합니다.
...
BroadcastReceiver가 작동하려면 AndroidManifest.xml에 등록해야 합니다
...
이 코드는 클릭 노출 횟수 추적의 구현을 기반으로 합니다. [의도]를 사용하여 이제 추적 정보를 Campaign으로 다시 보내야 합니다. 이 경우, 열기 의도를 앱의 특정 보기로 설정해야 하는 경우, 이 경우 의도 개체의 알림 데이터를 사용하여 onResume 메서드를 호출합니다.
...
앱이 백그라운드에 있는 동안 임프레션 추적이 계속 작동하도록 하려면 앱에서 추적 **을 수행해야** 한다는 점을 앱에 알리기 위해 사용 가능한콘텐츠를 전송해야 합니다.

>[!CAUTION]
>
>iOS 노출 횟수 추적은 정확하지 않으며 신뢰할 수 있는 것으로 간주해서는 안 됩니다.
...
그런 다음 [해제]를 처리하고 추적 정보를 전송하려면 다음을 추가해야 합니다.
