...
>[!NOTE]
>
>다음 **SMS 커넥터 프로토콜 및 설정** Adobe Campaign Classic용 은 다음 위치에 있습니다. [페이지](https://experienceleague.adobe.com/docs/campaign-classic/using/sending-messages/sending-messages-on-mobiles/sms-protocol.html).
>
>이 문서를 통해 프로토콜, 필드 이름 및 값에 대한 자세한 내용은 [SMPP 3.4 사양](https://smpp.org/SMPP_v3_4_Issue1_2.pdf).
...
* 텍스트는 140바이트로 제한되어 있으며, 인코딩에 따라 70~160자 사이의 문자를 의미합니다. 자세한 내용은 [SMS 텍스트 인코딩](../../administration/using/sms-protocol.md#sms-text-encoding) 아래에 자세히 및 제한 사항이 나와 있습니다.

* 수신자 주소(경우에 따라 호출됨) `ADC` 또는 `MSISDN`. SMS를 수신할 모바일 수입니다.

* 발신자 주소, 호출 가능 `oADC` 또는 경우에 따라 `sender id`. 이는 하루 종일 사용할 수 있는 전화 번호이거나, 공급자 또는 이름을 통해 보낼 때 짧은 코드일 수 있습니다. 이름은 선택 사항이며, 이 경우 SMS에 회신할 수 없습니다.
...
* A `data_coding` 필드의 인코딩을 나타냅니다.

## SMPP 프로토콜 {#smpp-protocol}

Adobe Campaign Standard은 SMPP 프로토콜 버전 3.4를 지원합니다. SMSC(공급자)로 SMS를 전송하고 SMS와 영수증을 받을 수 있는 광범위한 프로토콜입니다. 자세한 내용은 [SMPP 설명서](https://smpp.org/SMPP_v3_4_Issue1_2.pdf).
...
SMPP 전송 장치(&quot;패킷&quot;)는 PDU라고 합니다. A **PDU** 명령, 상태, 시퀀스 번호 및 데이터를 포함합니다.

각 PDU는 `SMPP RESP PDU` (동기 응답). 요청을 파이프할 수 있습니다. 발신자는 많은 명령을 기다리지 않고 보낼 수 있습니다 `RESP`를 사용하면 언제든지 파이프할 수 있는 요청 수를 창이라고 합니다. `RESP PDU` 해당 이니시에이터 PDU의 순서와 관련 없는 순서에 상관없이 도착할 수 있습니다.

분리된 상태로 **송신기+수신기** 모드, 사용되는 연결은 전송되는 메시지의 종류에 따라 다릅니다. 상기 송신기 접속은 MT에 사용되고, 상기 수신기 접속은 MO 및 SR에 사용된다. 각 메시지의 요청 및 응답은 동일한 TCP 연결을 통해 전송됩니다.

예를 들어, MT를 전송할 때 송신기 연결이 사용되고 `RESP` 그것은 MT가 또한 전송 채널을 통해 전송된다는 것을 인정합니다. MO(또는 SR)를 받으면, 수신자 연결은 MO를 수신하고 을 전송하는 데 사용됩니다 `RESP` 그것은 MO를 인정하고 있습니다.

![](assets/do-not-localize/sms_protocol_1.png)

Adobe Campaign Standard에서 MT 및 SR 조정은 MTA가 기본이므로 전용 SMS 프로세스가 없습니다.

성공 `SUBMIT_SM_RESP PDU` 성공하면 전송 로그에서 &quot;보낸 사람&quot; 메시지 상태를 트리거합니다 `DELIVER_SM (SR) PDU` &quot;수신됨&quot; 메시지 상태를 트리거합니다.
...
커넥터는 시스템에서 제공하는 기본 인증서를 사용합니다 `openssl` 라이브러리. 일반적으로 다음을 통해 제공됩니다 `/etc/ssl/certs` Debian의 디렉터리입니다. 이 디렉토리는 기본적으로 &quot;ca-certificates&quot; 패키지에서 제공되지만 사용자 지정할 수 있습니다.

### 각 PDU의 정보 {#information-pdu}

각 종류의 PDU에는 서로 다른 정보를 전달하는 개별 필드가 있습니다. 이러한 PDU는 [SMPP 3.4 사양](https://smpp.org/SMPP_v3_4_Issue1_2.pdf).

아래의 각 섹션에서는 PDU와 동기 응답 (`*_RESP PDU`). 모든 PDU는 해당 `RESP`, 사양의 필수 부분입니다.

PDU에는 선택적 필드가 있을 수 있습니다. 가장 일반적인 필드만 여기에 설명되어 있습니다. 자세한 내용은 [SMPP 3.4 사양](https://smpp.org/SMPP_v3_4_Issue1_2.pdf) 추가 정보.

#### BIND_TRANSMITTER / BIND_RECEIVER / BIND_TRANSMISVER {#bind-transmitter}

이 PDU는 SMSC에 대한 연결을 시작하는 데 사용됩니다. **송신기**, **받는 사람** 및 **트랜시버** 모드는 이 연결을 통해 전송할 수 있는 SMS 유형만 변경합니다. 특히 다음과 같습니다.
...
* **addr_ton** 및 **addr_npi**: 일부 공급자가 필요로 합니다. 에 의해 설정됨 `Bind TON` 및 `Bind NPI` 외부 계정의 설정.
...
이 PDU는 연결을 끊기 전에 시스템에서 전송해야 합니다. 일치하는 항목을 기다려야 합니다 `UNBIND_RESP PDU` 연결을 닫기 전에
...
의 주목할 만한 필드 `SUBMIT_SM PDU`:

* **service_type**: 일부 공급자가 필요로 합니다. 게재 속성에서 을 설정합니다.

* **source_addr_ton** 및 **source_addr_npi**: 전송된 소스 주소의 종류를 나타냅니다. 이러한 필드의 의미는 표준화되어 있지만, 일부 공급자가 이 필드를 다르게 사용하므로 공급자에게 올바른 값을 요청해야 합니다. 외부 계정에서 설정합니다.

* **source_addr**: MT의 소스 주소/ADC입니다. 휴대전화에 표시될 예정입니다 외부 계정 및 게재에서 설정하면 게재 값이 외부 계정 값보다 우선합니다.

* **dest_addr_ton** 및 **dest_addr_npi**: 전송할 대상 주소 종류를 나타냅니다(예: 로컬 또는 국제 형식). 이러한 필드의 의미는 표준화되어 있지만, 일부 공급자가 이 필드를 다르게 사용하므로 공급자에게 올바른 값을 요청해야 합니다. 외부 계정에서 설정합니다.

* **destination_addr**: 수신자 주소, 전화 번호 또는 MSISDN

* **esm_class**: 텍스트 필드에 UGH가 사용되는지 여부를 확인하는 데 사용됩니다. SMS 분할을 위해 커넥터가 자동으로 활성화한 경우 `message_payload` 모드가 사용되지 않습니다.

* **priority_flag**: 다른 메시지보다 우선합니다. 게재 자체의 우선순위에 연결되어 있습니다.

* **validation_period**: 다시 시도하지 않을 타임스탬프. 게재 자체에 를 설정합니다.

* **register_delivery**: 는 SR 요청 여부를 알려줍니다. Adobe Campaign은 자동 답글을 제외하고 항상 이 플래그를 설정합니다. 다중 부분 메시지의 경우 첫 번째 부품에 대해서만 플래그가 설정됩니다. 모든 버전은 동일한 동작이 있습니다.
...
* **message_payload**: 외부 계정에서 활성화되면 긴 메시지가 단일 PDU로 전송되고 텍스트가 아닌 이 필드에 전송됩니다 `short_message` 필드.
...
>[!IMPORTANT]
>
>많은 공급자가 MT ID를 16진수로 전송합니다. 를 반드시 설정해야 합니다. **MT 확인의 ID 형식** 외부 계정에서 올바로 설정됩니다.

일부 공급자가 `SUBMIT_SM_RESP` SR을 보낸 후 Adobe Campaign은 이 행동에 대해 답변하기 전에 30초 동안 기다립니다 **잘못된 메시지 ID** 알 수 없는 ID를 가진 SR에 연결할 수 없습니다.

#### DELIVERY_SM {#delivery-sm}

이 PDU는 SMSC에서 Adobe Campaign으로 전송됩니다. MO 또는 SR이 포함되어 있습니다.

대부분의 필드는 해당 필드와 같은 의미를 가지고 있습니다 `SUBMIT_SM` 상대. 다음은 유용한 필드 목록입니다.
...
* **short_message**: 메시지의 텍스트입니다. SR의 경우, SMPP 프로토콜 사양의 부록 B에 설명된 데이터가 포함됩니다. 자세한 내용은 [SR 오류 관리](../../administration/using/sms-protocol.md#sr-error-management) 자세한 내용

Adobe Campaign에서는 `receipted_message_id` 일부 구성 조정이 있는 선택적 필드.

#### DELIVERY_SM_RESP {#deliver-sm-resp}

이 PDU는 Adobe Campaign에서 SR 및 MO를 승인하도록 전송됩니다.

Adobe Campaign Standard은 `DELIVER_SM_RESP` 모든 처리 단계가 성공적으로 수행되면 이렇게 하면 처리 오류가 발생할 위험이 있는 동안 SR 또는 MO가 확인되지 않을 수 있습니다.
...
다중 부분 SMS 또는 긴 SMS는 여러 부분으로 전송되는 SMS입니다. 모바일 네트워크 프로토콜의 기술 제한 사항으로 인해 SMS는 140바이트보다 크지 않거나 분할해야 합니다. 자세한 내용은 [SMS 텍스트 인코딩](../../administration/using/sms-protocol.md#sms-text-encoding) SMS에 맞는 글자 수에 대해 자세히 알아보십시오.
...
를 사용하여 게재당 최대 SMS 수를 설정할 수 있습니다 **메시지당 최대 SMS 수** 설정 **게재 템플릿**. SMS가 너무 긴 실패 이유로 이 제한을 초과하는 메시지는 보내는 동안 실패합니다.

긴 SMS를 전송하는 방법에는 두 가지가 있습니다.

* **UGH**: 긴 메시지를 보내는 기본 및 권장 방법입니다. 이 모드에서는 커넥터가 메시지를 여러 개로 분할합니다 `SUBMIT_SM PDU`UDHH 정보가 들어 있습니다. 이 프로토콜은 휴대폰 자체에서 사용하는 것입니다. 즉, Adobe Campaign은 메시지 생성을 가장 많이 제어하므로 전송된 부분 수와 분할된 방법을 정확히 계산할 수 있습니다.

* **message_payload**: 전체 긴 메시지를 하나의 `SUBMIT_SM PDU`. 공급자가 분할해야 하므로 Adobe Campaign에서 얼마나 많은 부품을 전송했는지 정확히 알 수 없습니다. 일부 공급자는 이 모드를 필요로 하지만 UGH를 지원하지 않는 경우에만 사용하는 것이 좋습니다.

의 설명을 참조하십시오. `esm_class`, `short_message` 및 `message_payload` 의 필드 [SUBMIT_SM PDU](../../administration/using/sms-protocol.md#information-pdu) 를 참조하십시오.

### 처리량 최대 가용량 및 창 {#throughput-capping}

대부분의 공급자는 각 SMPP 연결에 대해 처리량 제한이 필요합니다. 외부 계정에서 많은 SMS를 설정하여 이를 수행할 수 있습니다. 연결당 처리량 조절이 수행되며 총 유효 처리량은 연결당 제한과 총 연결 수를 곱한 것입니다. 세부 사항은 [동시 연결](../../administration/using/sms-protocol.md#connection-settings) 섹션을 참조하십시오.

가능한 최대 처리량에 도달하려면 최대 전송 기간을 미세 조정해야 합니다. 전송 창은 `SUBMIT_SM PDU`대기 없이 보낼 수 있는 `SUBMIT_SM_RESP`. 자세한 내용은 [창 보내기 설정](../../administration/using/sms-protocol.md#throughput-timeouts) 섹션을 참조하십시오.

### SR 및 오류 관리(&quot;부록 B&quot;) {#sr-error-management}

SMPP 프로토콜은 `RESP PDU`그러나 SR에 대한 오류 코드는 정의하지 않습니다. 각 공급자는 자체 오류 코드를 의미와 함께 사용합니다.

권장 사항은 [SMPP 프로토콜 사양](https://smpp.org/SMPP_v3_4_Issue1_2.pdf) (페이지 167) 실제 오류 코드나 그 의미를 나열하지 않습니다.

오류 관리에 맞게 Adobe Campaign의 브로드로그 메시지 시스템을 활용하여 오류 및 심각도(하드, 소프트 등)를 적절하게 프로비저닝했습니다.

위에서 언급했듯이, 두 가지 다른 종류의 오류가 있습니다.

* 의 동기 응답 `SUBMIT_SM_RESP` 메시지가 SMSC에 전송된 직후 발생합니다
* 휴대폰에서 메시지를 받거나 메시지 시간이 초과되었을 때 훨씬 나중에 발생할 수 있는 영수증. 이 경우 SR에 오류가 있습니다.

SR이 수신되면 상태 및 오류를 해당 위치에서 찾을 수 있습니다 `short_message` 필드(부록 B 준수 구현의 경우). 다음 `short_message` PDU의 필드를 종종 라고 합니다 **텍스트 필드** MT에 텍스트가 포함되어 있으므로 SR의 경우 기술 정보와 라는 하위 필드를 포함합니다 **텍스트**. 이 두 필드는 서로 다르며 `short_message` 실제로 를 포함합니다 **텍스트** 필드 및 기타 정보.
...
id 필드는 ID가 `SUBMIT_SM_RESP PDU`MT의 승인.

`sub` 및 `dlvrd` 는 전달된 부분 및 전달된 메시지의 양을 계산해야 하지만 브로드로그 시스템에서 더 나은 통합 정보를 제공하므로 Adobe Campaign에서 사용하지 않습니다.
...
통계 필드는 메시지의 상태를 알려주기 때문에 중요합니다. 중요한 상태는 `DELIVRD`, `UNDELIV` 및 `REJECTD`. 다음 `DELIVRD` 상태는 성공을 나타내고 다른 두 가지는 오류를 나타냅니다. 다른 값을 사용할 수 있지만 일반적으로 중간 알림입니다(예: MT가 이동통신사에 도달했지만 휴대전화에는 도달하지 않음). Adobe Campaign에서는 이러한 중간 알림을 무시합니다.
...
그런 다음 `status extraction` regex 및 `error code extraction` regex는 이러한 필드를 추출하기 위해 적용되며 문자열에 추가됩니다.
...
메시지가 브로드로그 메시지 테이블에 아직 제공되지 않으면 전체 메시지를 **firstText** 및 정규화된 메시지입니다. 그런 다음 커넥터가 성공을 사용하고 `error` regex에서 성공 또는 실패인지를 확인합니다.

* 와 일치하는 경우 `success` regex, 그것은 성공으로 간주될 것입니다.

* 와 일치하는 경우 `error` regex에서 메시지는 오류로 간주됩니다.
...
다음을 수행해야 합니다. **인코딩 문제가 발생하면 항상 SMSC 공급자에게 문의하십시오**. SMSC 공급자만 지원하는 인코딩과 기술 플랫폼의 제한 사항으로 인해 적용될 수 있는 특수 규칙에 대해 정확하게 알고 있습니다.

SMS 메시지는 GSM7 인코딩이라고 하는 특수 7비트 인코딩을 사용합니다.

SMPP 프로토콜에서 GSM7 텍스트가 문자당 8비트로 확장되어 문제를 더 쉽게 해결할 수 있습니다. SMSC는 모바일로 전송되기 전에 문자당 7비트로 묶습니다. 이것은 `short_message` SMS의 필드는 SMPP 프레임에서 최대 160바이트일 수 있지만 모바일 네트워크에서 전송할 때는 140바이트로 제한됩니다.
...
* **SMS에서는 ASCII를 사용하지 마십시오.** SMSC 공급자가 명시적으로 요청한 경우를 제외하고. 이 인코딩은 GSM7보다 8비트 문자를 사용하고 적용 범위가 적으므로 공간을 낭비합니다. 이 인코딩은 북미에서 사용되는 CDMA 네트워크에 필요할 수 있습니다.

* Latin-1이 항상 지원되는 것은 아닙니다. Latin-1을 사용하기 전에 SMSC 공급자와의 호환성을 확인하십시오.

* 국가 언어 이동 표는 Adobe Campaign 커넥터에서 지원하지 않습니다. UCS-2 또는 기타 를 사용해야 합니다 `data_coding` 을 가리키도록 업데이트하는 것이 좋습니다.
...
다음 `data_coding` 필드는 사용할 인코딩을 알려줍니다. 가장 큰 문제는 값 0이 일반적으로 GSM7을 참조하는 사양에서 기본 SMSC 인코딩을 의미한다는 것입니다. 인코딩이 연결된 SMSC 파트너에게 문의하십시오 `data_coding` = 0. Adobe Campaign만 지원합니다. 기타 `data_coding` 값은 사양을 따르는 경향이 있지만 SMSC 공급자에게 확인하는 것이 유일한 방법입니다.
...
이 옵션을 사용하면 연결 수를 보다 세밀하게 제어할 수 있습니다. [동시 연결](../../administration/using/sms-protocol.md#connection-settings).
...
연결을 설정합니다. **송수신 장치** 모드 또는 분리됨 **송신기+수신기** 모드. 분리 **송신기+수신기** 모드, 설정 **SMPP 연결 모드** 섹션은 의 송신기 및 설정에 적용됩니다 **받는 사람 연결 설정** 섹션을 선택한 경우에만 수신자 연결에 적용됩니다 **수신기에 대해 다른 매개 변수 사용** 확인란을 선택합니다.

#### SMSC 구현 이름 {#smsc-implementation-name}

SMSC 구현의 이름을 설정합니다. 공급자의 이름으로 설정해야 합니다. 이 필드에 추가할 내용을 알려면 관리자 또는 게재 가능성 팀에 문의하십시오. 이 필드의 역할은 [SR 오류 관리](../../administration/using/sms-protocol.md#sr-error-management) 섹션을 참조하십시오.
...
연결의 로그인입니다. 에 전달됨 `system_id` 바인딩 PDU의 필드.
...
에서 전달된 값 `system_id` 바인딩 PDU의 필드. 일부 공급자는 여기에서 특정 값이 필요합니다.
...
구분 **송신기/수신기** mode 로, 위의 연결 수는 **송신기/수신기** 쌍은 총 연결 수의 두 배가 됨을 의미합니다.
...
상자를 선택하면 **연결 설정** 섹션은 의 송신기 및 설정에 적용됩니다 **받는 사람 연결** 설정이 수신기에 적용됩니다.

**받는 사람 서버, 포트, 계정, 암호, 시스템 유형**

이 설정은 로그인할 때 수신기에 적용됩니다 **송신기+수신기** 모드. 자세한 내용은 위의 내용을 참조하십시오.
...
자세한 내용은 [인코딩 설정의 특정 매핑 정의](../../administration/using/sms-protocol.md#SMSC-specifics) 를 참조하십시오.
...
이러한 단점은 데이터베이스 경합 때문에 성능이 저하될 수 있습니다. 비활성화하면 **syncfromexec** 워크플로우에서 20분마다 실행됩니다.
...
TON(숫자 유형) 및 NPI(번호 지정 계획 지시자)는 [SMPP 3.4 사양](https://smpp.org/SMPP_v3_4_Issue1_2.pdf) (117페이지). 이러한 값은 공급자의 필요에 따라 설정해야 합니다.

그대로 전송됩니다 `source_addr_ton`, `source_addr_npi`, `dest_addr_ton` 및 `dest_addr_npi` 의 필드 `SUBMIT_SM PDU`.

#### 서비스 유형 {#service-type}

이 필드는 `service_type` 필드 `SUBMIT_SM PDU`. 공급자의 필요에 따라 설정합니다.
...
이 창은 네트워크 링크가 지연 시간이 높을 때 처리량을 늘리는 데 도움이 됩니다.  창의 값은 SMS 수와 링크의 지연 시간(초)을 곱한 값 이상이어야 커넥터가 를 기다리지 않도록 합니다 `SUBMIT_SM_RESP` 다음 메시지를 보내기 전에
창이 너무 크면 연결 문제가 발생할 경우 중복 메시지를 보낼 수 있습니다. 또한, 대부분의 제공업체는 윈도우에 매우 엄격한 제한을 두고 한도를 초과하는 메시지를 거절한다.

최적의 전송 창 공식을 계산하는 방법:

* 사이의 최대 지연 시간을 측정합니다. `SUBMIT_SM` 및 `SUBMIT_SM_RESP`.

* 이 값을 최대 MT 처리량에 초 단위로 곱합니다. 이를 통해 최적의 전송 창 값이 제공됩니다.

예: 최대 MT 처리량에 300개의 SMS/s가 설정되어 있고 그 사이에 100ms 지연이 있는 경우 `SUBMIT_SM` 및 `SUBMIT_SM_RESP` 평균적으로 최적의 값은 `300×0.1 = 30`.
...
자세한 내용은 [SMS 텍스트 인코딩](../../administration/using/sms-protocol.md#sms-text-encoding) 텍스트 인코딩에 대한 세부 정보 섹션을 참조하십시오.

이 설정을 사용하면 지정과 다른 사용자 지정 인코딩 매핑을 정의할 수 있습니다. 인코딩 목록과 함께 선언할 수 있습니다 `data_coding` 값.

MTA는 목록의 첫 번째 인코딩을 사용하여 인코딩을 시도합니다. 실패하면 목록의 다음 인코딩을 사용하려고 합니다. 메시지를 인코딩하는 데 인코딩할 수 없는 경우 오류가 발생합니다. 인코딩이 발견되면 MTA가 `SUBMIT_SM PDU` 인코딩된 텍스트와 `data_coding` 표에 지정된 값으로 설정된 필드입니다.
...
이는 MTA가 GSM에서 메시지를 인코딩하려고 함을 의미합니다. 성공하면 `data_coding` 0으로 설정합니다.

메시지를 GSM으로 인코딩할 수 없는 경우 UCS-2로 인코딩되고 설정됩니다 `data_coding` 8.

#### message_payload 활성화 {#enable-message-payload}

선택 취소하면 긴 SMS가 MTA로 분할되고 여러 개로 전송됩니다 `SUBMIT_SM PDU`UGH를 사용합니다. 이 메시지는 UGH 데이터에 따라 휴대폰에 의해 다시 작성됩니다.

이 옵션을 선택하면 긴 SMS가 하나의 SUBMIT_SM PDU에 전송되고 message_payload 옵션 필드에 텍스트가 표시됩니다. 자세한 내용은 [SMPP 사양](../../administration/using/sms-protocol.md#ACS-SMPP-connector) 자세한 내용은

이 기능이 활성화되어 있으면 Adobe Campaign에서 SMS 부분을 개별적으로 카운트할 수 없습니다. 모든 메시지는 한 부분으로 전송됩니다.

#### 전체 전화 번호 보내기 {#send-full-phone-number}

이 확인란을 선택하지 않으면 전화 번호의 자릿수만 공급자에게 전송됩니다(`destination_addr` 필드 `SUBMIT_SM` 필드)만 로드하는 것입니다. SMPP의 TON 및 NPI 필드로 대체되는 국제 번호 지표(일반적으로 + 접두어)가 대체되므로 이 동작은 기본 동작입니다.
...
TON(숫자 유형) 및 NPI(번호 지정 계획 지시자) [SMPP 3.4 사양](https://smpp.org/SMPP_v3_4_Issue1_2.pdf) (117페이지). 이러한 값은 공급자가 필요로 하는 대로 설정해야 합니다.

그대로 전송됩니다 `addr_ton` 및 `addr_npi` 바인딩 PDU 필드
...
**이 값은 해결 방법으로 문제 해결 목적에만 사용해야 합니다** 를 설정하고 을(를) 일반 조건으로 0으로 설정합니다.

Fox 예: 를 2로 설정할 때

* 공급자가 SR(`DELIVER_SM`). ID가 &quot;1234&quot;인 경우

* 데이터베이스에서 ID &quot;1234&quot;를 찾을 수 없습니다.

* 커넥터는 카운트 1입니다 **잘못된 ID** 오류가 발생하여 `DELIVER_SM_RESP` 를 &quot;메시지 ID가 잘못되었습니다&quot; 오류 코드(일반 동작)로 바꿉니다.

* 공급자가 ID가 &quot;1234&quot;인 동일한 SR을 다시 시도합니다.

* 데이터베이스에서 ID &quot;1234&quot;를 찾을 수 없습니다.

* 커넥터는 카운트 2입니다 **잘못된 ID** 오류가 발생하여 `DELIVER_SM_RESP` &quot;OK&quot;(제대로 처리되지 않았더라도)

* 이 기능은 잘못된 SR 블록이 메시지를 처리할 수 없는 경우 공급자 측에서 SR 버퍼를 플러시하기 위한 것입니다.

이 필드를 0으로 설정하면 **메시지 ID가 잘못되었습니다.** 가 항상 반환되고, 이는 일반적인 동작입니다.
...
SR 형식은 SMPP 프로토콜 사양에서 엄격하게 적용되지 않습니다. 에 설명된 권장 사항일 뿐입니다 [부록 B](../../administration/using/sms-protocol.md#sr-error-management) (167페이지) 사양을 포함합니다. 일부 SMPP 구현자는 이 필드의 형식을 다르게 지정하므로 Adobe Campaign에서 올바른 필드를 추출하는 방법이 필요합니다.

기본적으로 다음 문자 뒤에 최대 10자의 영숫자를 캡처합니다 `id:`.

regex에는 괄호 안에 포함된 부분이 있는 정확히 하나의 캡처 그룹이 있어야 합니다. 괄호는 ID 부분을 둘러싸야 합니다. regex 형식은 PCRE입니다.

이 설정을 조정할 때에는 잘못된 트리거를 방지하기 위해 가능한 한 많은 컨텍스트를 포함해야 합니다. 특정 접두사가 있는 경우(예: ) `id:` 표준에서 regex에 포함하십시오. 단어 구분 기호(\b)도 단어 중간에 텍스트를 캡처하지 않도록 가능한 한 많이 사용합니다.
...
기본적으로 다음으로 시작하는 시작 값 `DELIV`예: `DELIVRD` 에서 [부록 B](../../administration/using/sms-protocol.md#sr-error-management)은 성공적으로 전달되었다고 간주되고, 오류와 일치하는 모든 상태 값으로 간주됩니다(예: ). `REJECTED`, `UNDELIV`은 오류로 간주됩니다.

#### MT 확인의 ID 형식 {#id-format-mt}

다음에서 반환된 ID의 형식을 나타냅니다 `message_id` 필드 `SUBMIT_SM_RESP PDU`.
...
* **16진수 문자열**: ID는 16진수로 인코딩된 바이트 문자열인 ASCII 인코딩된 텍스트여야 합니다. 예를 들어, PDU에서 `0x34 0x31 0x34 0x32 0x34 0x33`는 ASCII &quot;414243&quot;으로 변환됩니다. 이 문자열은 16진수 바이트 문자열로 디코딩되고 결과적으로 &quot;ABC&quot;를 가져옵니다. ID &quot;ABC&quot;를 데이터베이스에 저장합니다.
...
이 확인란을 선택하면 선택적 필드의 콘텐츠가 위의 레지스트리에서 처리한 텍스트에 추가됩니다. 텍스트 형식은 다음과 같습니다 `0xTAG:VALUE`, `0xTAG` 태그의 4자리 16진수 값(예: ) `0x002E`.

예를 들어 `receipted_message_id` 필드. 이 경우 이 확인란을 활성화하면 다음 텍스트가 해당 상태에 추가됩니다.
...
이 옵션을 선택하면 **텍스트** 필드는 SR의 상태 텍스트를 처리하는 동안 유지됩니다.
...
이 옵션을 활성화하면 `Extraction` SR 필드에 있는 ID의 regex가 충분하지 않습니다. 의 컨텐츠 **텍스트** 필드를 ID로 구문 분석할 수 있으며 공격자가 이 필드를 사용하여 위조 ID를 주입할 수 있으므로 서비스 거부 상황이 일부 발생할 수 있습니다.

**서비스 ID 태그**

사용자 지정 TLV를 추가할 수 있습니다. 이 필드는 태그 부분을 설정합니다. 값은에서 게재당 사용자 지정할 수 있습니다 **서비스 또는 프로그램 ID** 값을 사용할 수 있습니다.
...
다음 **키워드** 및 **짧은 코드** 열은 자동 답장을 트리거할 조건을 정의합니다. 두 필드가 모두 일치하면 MO가 전송되고 추가 작업이 트리거됩니다. 와일드카드를 지정하려면 필드를 비워 두어야 합니다. 키워드는 MO 텍스트의 첫 번째 영숫자 단어와 일치하고 구두점과 선행 공백을 무시합니다. 즉, **키워드** 필드에는 공백을 포함할 수 없으며 단일 단어여야 합니다.

다음 **키워드** 설정은 접두어입니다. 예를 들어 &quot;AD&quot;를 지정하는 경우 &quot;AD&quot;, &quot;ADAPT&quot; 및 &quot;ADOBE&quot;과 일치합니다. 공통 접두사가 있는 키워드가 여러 개 있는 경우 키워드가 위에서 아래로 처리되므로 순서에 주의를 기울여야 합니다.

다음 **회신** 열은 응답할 텍스트입니다. 이 필드에는 개인화를 사용할 수 없습니다. 이 필드를 비워 두면 메시지가 회신되지 않지만 추가 작업이 트리거됩니다.

다음 **추가 작업** 열은 둘 다 **키워드** 및 **짧은 코드** 일치하면 빈 짧은 코드가 모든 짧은 코드와 일치합니다. 격리에 보내거나 격리에서 제거할 수 있습니다. 텍스트에 회신하지 않는 값을 지정합니다. 를 지정하는 경우 **추가 작업** 하지만 **회신** 필드가 비어 있으면 작업이 실행되지만 응답이 전송되지 않습니다. 격리는 지정된 짧은 코드에만 적용되거나, 필드가 비어 있는 경우 모든 짧은 코드에만 적용됩니다.
...
21.1 릴리스부터 자동 회신 MT에 선택적 매개 변수를 추가할 수 있습니다. 선택적 TLV 매개 변수로서 가 `SUBMIT_SM PDU` 응답 중, [SMPP 사양](https://smpp.org/SMPP_v3_4_Issue1_2.pdf)(131페이지).
...
이 필드는 선택 사항입니다. ADC(발신자 주소)를 재정의할 수 있습니다. 이 필드의 콘텐츠는 `source_addr` 필드 `SUBMIT_SM PDU`.
...
이 설정은 **메시지 페이로드** 설정이 비활성화되어 있습니다. 자세한 내용은 다음을 참조하십시오 [페이지](../../administration/using/configuring-sms-channel.md). 메시지에 이 값보다 SMS가 더 많은 경우 오류가 트리거됩니다.
...
* **지정되지 않음** PDU에 옵션 필드를 보내지 않습니다.

* **Flash** 값을 1로 설정합니다. 이 플러그인은 모바일에서 튀어나오고 메모리에 저장되지 않는 플래시 메시지를 보냅니다.

* **일반** 값을 0으로 설정합니다. 일반 메시지를 보냅니다.

* **모바일에 저장** 값을 2로 설정합니다. 휴대폰에 SMS를 내부 메모리에 저장하라고 지시합니다.

* **터미널에 저장** 값을 3으로 설정합니다. 휴대전화에 SMS를 심카드에 저장하도록 되어 있다.

#### 유효 기간 {#validity-period}

유효 기간은 `validity_period` 필드 `SUBMIT_SM PDU`. 날짜는 항상 절대 UTC 시간 형식으로 지정됩니다(날짜 필드는 &quot;00+&quot;로 끝남).

#### SMPP 선택적 매개 변수(TLV) {#smpp-optional-parameters}

21.1 릴리스부터 이 게재에 대해 전송되는 모든 MT에 여러 선택적 매개 변수를 추가할 수 있습니다. 이러한 선택적 매개 변수는 `SUBMIT_SM PDU` 응답 중, [SMPP 사양](https://smpp.org/SMPP_v3_4_Issue1_2.pdf)(131페이지).
...
선택적 매개 변수에 동일한 **태그 Id** 로서의 **서비스 태그 Id** 외부 계정에 정의되면 이 표에 정의된 값이 우선합니다.
...
공급자가 제공한 ID는에 저장됩니다. `sProviderId` 열 `nmsBroadLogExec` 테이블. SR은 MT가 성공적으로 전송되고 인식되면 항상 도착하지만, Adobe Campaign에서 뛰어난 SR로 알려져 있으며, 때때로 순서가 잘못될 수 있습니다. 처리 스레드는 전체 정보가 도착할 때까지 이러한 SR을 일시적으로 RAM에 저장합니다.

MT가 확인되면 (`SUBMIT_SM_RESP`), `sProviderId` 는 즉시 데이터베이스에서 업데이트됩니다.
...
* ID가 검색됩니다. `nmsBroadLogExec:sProviderId`.
* 상태 + 오류 코드는 레지엑스를 사용하여 SR에서 추출됩니다.
* 브로드로그 메시지 메커니즘은 오류를 식별하고 브로드로그 메시지 ID를 찾는 데 사용됩니다.
* 브로드로그는 위의 모든 정보로 업데이트됩니다.
* SR이 인식됩니다.

위의 단계를 확인하려면 다음을 수행해야 합니다 **자세한 SMPP 추적 사용** 모든 단계가 올바르게 적용되었는지 수동으로 확인합니다. Adobe Campaign이 새 SMPP 공급자에 연결할 때마다 필요합니다.
...
이전 SMS 외부 계정이 없는지 확인합니다. 테스트 계정을 비활성화한 상태로 두면 운영 시스템에서 다시 활성화하여 잠재적인 충돌을 생성할 수 있는 위험을 실행합니다.
...
GSM이 아닌 문자 또는 ASCII가 아닌 문자가 있는 SMS를 보내야 하는 경우 다양한 문자가 포함된 메시지를 가능한 한 많이 보내십시오. 사용자 지정 문자 매핑 테이블을 설정하는 경우 가능한 모든 항목에 대해 하나 이상의 SMS를 전송하십시오 
`data_coding` values.

* **SR이 제대로 처리되었는지 확인**
SMS는 게재 로그에서 수신으로 표시되어야 합니다. 게재 로그는 성공적이며 다음과 같습니다. 게재 공급자 이름을 변경했는지 확인합니다. 게재 로그에는 다음을 포함하지 않아야 합니다    `SR yourProvider stat=DELIVRD err=000|#MESSAGE`
게재 공급자 이름을 변경했는지 확인합니다. 게재 로그에는 다음을 포함하지 않아야 합니다 **SR 일반** 프로덕션 환경에서 작업하는 경우입니다.

* **MO가 처리되었는지 확인**
MO(자동 회신, 데이터베이스에 MO 저장 등)를 처리해야 하는 경우 몇 가지 테스트를 해 보세요. 모든 자동 회신 키워드에 대해 몇 개의 SMS를 보내고, 몇 초 이내에 회신이 충분한지 확인합니다.
...
확인 `BIND_* PDUs` 가 올바르게 전송됩니다. 확인해야 할 가장 중요한 사항은 공급자가 항상 성공적으로 반환된다는 것입니다 `BIND_*_RESP PDUs` (command_status = 0).

너무 많지는 않은지 확인해 보세요 `BIND_* PDU`s. 너무 많은 경우 연결이 불안정하다는 것을 나타낼 수 있습니다. 자세한 내용은 [불안정한 연결 문제](../../administration/using/sms-protocol.md#issues-unstable-connection) 섹션을 참조하십시오.

#### INQUIRE_LINK {#enquire-link-pdus}

확인 `ENQUIRE_LINK PDU`연결이 유휴 상태일 때 가 정기적으로 교환됩니다.
...
* 확인 `data_coding` 은 올바르며 기본적으로 0입니다.
* 확인 `short_message` 가 올바르게 인코딩되어 있습니다. 여러 인코딩을 지원하는 16진수 변환기를 사용하여 디코딩해 보십시오.

사용 `SUBMIT_SM_RESP PDU`:
...
* regex가 정의된 regex 확인 도구를 확인합니다. `Extraction` SR에서 ID의 regex는 정확히 하나의 캡처 그룹을 반환하며 메시지의 전체 ID를 캡처합니다.
* 추출된 ID가 의 ID와 일치하는지 확인합니다 `SUBMIT_SM_RESP`.
* 에 정의된 정규식을 확인합니다. `Extraction` SR에서 상태의 regex는 stat 필드의 컨텐츠를 반환합니다.
* 에 정의된 정규식을 확인합니다. `Extraction` SR에서 오류의 regex는 오류 필드의 내용을 반환합니다.

사용 `DELIVER_SM_RESP PDU`:

* 을 받은 후 전송되었는지 확인합니다 `DELIVER_SM PDU`: 일반적으로 1초 미만.
* 성공, command_status = 0인지 확인합니다.
...
모든 확인이 완료되면 마지막 작업은 **자세한 SMPP 추적 비활성화** 너무 많은 로그를 생성하지 않습니다. 프로덕션 시스템에서도 문제 해결 목적으로 다시 활성화할 수 있습니다.
