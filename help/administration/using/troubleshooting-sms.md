...
[에서 사용할 수 `BIND PDU`있는 필드를 확인하려면 위의 각 PDU](../../administration/using/sms-protocol.md#information-pdu) 종류에 있는 정보 섹션을 참조하십시오. 일반적으로 에 단축 코드를 `address_range`추가하지만 공급자의 특별한 지원이 필요합니다. 여러 단축 코드를 독립적으로 라우팅하는 방법을 알려면 해당 업체에 문의하십시오.
