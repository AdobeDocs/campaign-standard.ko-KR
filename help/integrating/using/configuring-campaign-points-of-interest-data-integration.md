...
>[!NOTE]
>
>모바일 응용 프로그램은 Adobe Experience Platform SDK를 사용하여 Adobe Campaign Standard에 이미 구성되어 있어야 합니다. 자세한 내용은 이 [페이지를 참조하십시오](https://helpx.adobe.com/campaign/kb/configuring-app-sdk.html).
...
1. Adobe Campaign에서 모바일 애플리케이션 및 수집된 위치 데이터에 액세스해야 합니다. 위치 데이터 [를 수집하는 데 사용되는 모바일 앱](#accessing-mobile-apps-used-to-collect-location-data) 액세스 및 [수집된 위치 데이터 액세스를 참조하십시오](#accessing-collected-location-data).
...
1. 모바일용 Adobe Analytics에 액세스 자세한 내용은 라이선스 계약서를 확인하거나 Adobe 계정 담당자에게 문의하십시오.
1. Adobe Campaign에서 모바일 애플리케이션을 설정합니다. Campaign [에서 모바일 앱 설정을 참조하십시오](#setting-up-a-mobile-app-in-campaign).
1. Adobe Mobile Services 인터페이스에서 모바일 애플리케이션을 설정합니다. 이를 통해 Adobe Mobile Services에서 수집한 데이터가 Adobe Campaign으로 전송되도록 할 수 있습니다. Adobe [Mobile Services에서 모바일 앱 구성을 참조하십시오](#configuring-a-mobile-app-in-adobe-mobile-services).
...
Adobe Campaign을 통해 관심 영역 데이터를 수집하려면, 데이터를 받을 Adobe Campaign을 모바일 응용 프로그램을 구성해야 합니다.
...
* 이 **[!UICONTROL Collect PII endpoint]** URL은 모바일 응용 프로그램이 시작될 때 사용자의 Experience Cloud ID 및 등록 토큰을 수집하는 데 사용됩니다. 사용자가 이메일, 이름, 성 등과 같은 자격 증명을 사용하여 응용 프로그램에 로그인하면 이 데이터도 수집되어 Adobe Campaign 프로필과 사용자의 등록 토큰을 조정하는 데 사용됩니다.
...
Adobe Campaign에 설정된 모바일 애플리케이션 매개 변수에서 찾을 수 있는 특정 정보가 필요합니다(Campaign [에서 모바일 앱 설정 참조](#setting-up-a-mobile-app-in-campaign)).
...
1. 포스트백을 만듭니다.
   ...
   * 이 **[!UICONTROL URL]** 필드에서 Adobe Campaign 인터페이스에 구성된 모바일 응용 프로그램에서 서버 이름 앞에 **[!UICONTROL Collect PII Endpoint]** URL을 복사합니다. Campaign [에서 모바일 앱 설정을 참조하십시오](#setting-up-a-mobile-app-in-campaign).
...
1. 두 번째 포스트백을 만듭니다.
   ...
   * 이 **[!UICONTROL URL]** 필드에서 Adobe Campaign 인터페이스에 구성된 모바일 응용 프로그램에서 서버 이름 앞에 **[!UICONTROL Location Services Endpoint]** URL을 복사합니다. Campaign [에서 모바일 앱 설정을 참조하십시오](#setting-up-a-mobile-app-in-campaign).
   * 다음과 같이 **[!UICONTROL Post Body]** 필드를 입력합니다.
...
이 단계는 이 [페이지에 설명되어 있습니다](https://helpx.adobe.com/campaign/kb/configuring-app-sdkv4.html).
...
이 단계는 SDK V4 [를 사용하여 모바일 애플리케이션 구성 페이지에서 설명합니다](https://helpx.adobe.com/campaign/kb/configuring-app-sdkv4.html) .
