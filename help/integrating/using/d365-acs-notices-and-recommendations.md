...
연락처 및 사용자 지정 엔티티 동기화를 위해 이 통합은 을 처리합니다 **Microsoft Dynamics 365 를 진실의 소스로 사용**.  Any changes to synchronized attributes should be done in Dynamics 365 and not in Adobe Campaign Standard).  Campaign에서 변경된 경우 동기화가 한 방향으로 진행되므로 동기화 중에 Campaign에서 덮어쓸 수 있습니다.
...
양방향 옵트아웃 구성은 논리를 사용하여 두 시스템에 쓸 값을 결정합니다. The logic compares timestamps between the two systems (record-level change in Dynamics 365, attribute-level change in Campaign) to determine which system prevails. Campaign에 더 최근 타임스탬프가 포함되어 있으면 캠페인 값이 우선합니다. Dynamics 365에 최신 타임스탬프가 들어 있거나 타임스탬프가 같은 경우 opt-out=TRUE가 됩니다(값 중 하나가 TRUE라고 가정).
...
아래 사용 사례에서 통합으로 Campaign SFTP 저장소를 활용해야 합니다.  SFTP 계정에 이러한 사용 사례를 수용할 수 있는 적절한 스토리지 용량이 있는지 확인해야 합니다. Exceeding the licensed SFTP storage capacity may severely impair the functional use of Campaign, the integration and/or the SFTP account.
...
>[!IMPORTANT]
>
>부품에 대한 특정 작업(예: 레코드 초기 수집, 레코드 데이터 재재생 등) could result in a large load of records being ingested from Microsoft Dynamics 365 to your Adobe Campaign instance. 성능 문제의 위험을 줄이기 위해 모든 Campaign 프로세스를 중지하는 것이 좋습니다(예: 마케팅 활동, 워크플로우 실행 없음 등). 대량의 레코드를 Campaign에 수집하기 전까지.
...
* You will need to license the proper Campaign package to support the engine call volume generated by the integration. 라이선스가 있는 엔진 호출 볼륨을 초과하면 Campaign 성능이 저하될 수 있습니다.
...
* 통합은 개인 정보(예: GDPR)를 발행 하지 않습니다. 최종 사용자 개인 정보 보호 요청을 이행할 책임은 고객에게 있습니다. 이러한 요청은 Campaign(Adobe Experience Platform Privacy Service을 통해) 및 Dynamics 365를 모두 독립적으로 해야 합니다. The integration can issue regular deletes to help with data synchronization, if desired.   검토 [개인 정보 섹션](#manage-privacy-requests) 추가 정보.
...
* 통합은 다음을 수행합니다 **not** Dynamics 365와 Campaign Standard 데이터 모델 간의 데이터 리모델링을 지원합니다. It is expected that the integration will be syncing one Dynamics 365 table to one Campaign table.
