...
1. 이 **[!UICONTROL Application criteria]** 탭에서 이 규칙을 모든 게재에 적용하거나 전송할 메시지에 따라 규칙의 적용 가능성을 제한할 수 있습니다. 응용 프로그램 조건이 충족되는 경우에만 규칙이 실행됩니다. 예를 들어 지정된 단어로 시작하거나 특정 문자가 포함된 ID가 있는 메시지에만 규칙을 적용할 수 있습니다. 필터링 [규칙의](../../sending/using/filtering-rules.md#restricting-the-applicability-of-a-filtering-rule)적용 가능성 제한을 참조하십시오.

   ![](assets/fatigue20.png)

1. 탭을 **[!UICONTROL Typologies]** 선택하고 유형 규칙을 게재에 사용된 유형 분석에 연결합니다. 유형 [및 유형](../../sending/using/about-typology-rules.md#managing-typologies) 규칙 [관리를 참조하십시오](../../sending/using/about-typology-rules.md#typology-rules).
...
* **[!UICONTROL Refine threshold on profiles and deliveries > Deliveries to count]**:이 유형 규칙을 사용하는 모든 배달은 규칙을 실행하지만, 계산하려는 과거 및 예약된 배달 중에서 결정합니다. 예를 들어 &#39;Label starts with Newsletter&#39;가 제한되면 배달 레이블이 &#39;Promo&#39;로 시작되더라도 규칙이 실행됩니다. 선택한 슬라이딩 기간 동안 레이블이 &#39;Newsletter&#39;로 시작하는 배달 수를 계산합니다.
