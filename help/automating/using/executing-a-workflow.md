...
>[!CAUTION]
>
> Adobe 에서는 고객이 워크플로우 실행을 우선 순위를 정하고 최대 20 개의 동시 워크플로우 실행을 실행하여 인스턴스 전체에서 최대 성과를 일관되게 유지할 것을 권장합니다. 20 개 이상의 동시 워크플로우 실행이 계획될 수 있으며 기본적으로 순차적으로 실행됩니다. 고객 지원 팀에 티켓을 제출하여 최대 동시 워크플로우 실행 수에 대한 기본 설정을 조정할 수 있습니다.
