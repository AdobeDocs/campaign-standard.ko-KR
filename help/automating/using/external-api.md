...
이 **[!UICONTROL External API]** 활동은 HTTP API 호출을 통해 **외부 시스템에서** 워크플로우로 데이터를 **가져옵니다** .

외부 시스템 끝점은 몇 가지 카테고리를 언급할 때 공용 API 끝점, 고객 관리 시스템 또는 서버리스 응용 프로그램 인스턴스(예: Adobe [I/O 런타임](https://www.adobe.io/apis/experienceplatform/runtime.html))일 수 있습니다.
...
>[!NOTE]
>
>머리글 값이 활동 내에 마스크되어 있으므로 헤더 값을 복사할 수 없습니다.

그런 다음, 새로운 외부 API 활동의 데이터를 가리키거나/또는 새 외부 API 활동의 데이터를 사용하기 위해 베타 외부 API 활동을 가리키거나/또는 사용하는 워크플로우의 다른 활동을 다시 구성합니다. 활동의 예:이메일 전달(개인화 필드), 활동 강화 등
...
>[!CAUTION]
>
>활동은 대량의 데이터가 전송될 수 있으므로 각 프로필에 대한 특정 정보를 검색하는 것이 아니라 캠페인 전체 데이터(최신 오퍼 집합, 최신 점수 등)를 가져오기 위한 것입니다. 사용 사례에 필요한 경우 파일 전송 [활동을 사용하는 것이 좋습니다](../../automating/using/transfer-file.md) .


JSON에는 다음과 같은 특정 보장 조치가 있습니다.

* **JSON 최대 깊이**:10개 수준으로 처리할 수 있는 사용자 지정 중첩 JSON의 최대 깊이를 제한합니다.
* **JSON 최대 키 길이**:생성된 내부 키의 최대 길이를 255로 제한합니다. 이 키는 열 ID와 연결됩니다.
* **허용되는 JSON 최대 복제 키**: 열 ID로 사용되는 복제 JSON 속성 이름의 최대 총 수를 150개로 제한합니다.


활동은 다음과 같이 JSON 구조를 지원하지 않습니다.
...
카운트 **매개 변수** 추가 확인란을 선택하면 임시 테이블에서 나오는 각 행에 대한 카운트 값이 추가됩니다. 이 확인란은 인바운드 활동이 임시 테이블을 생성하는 경우에만 사용할 수 있습니다.
...
JSON 파서는 일부 예외를 제외하고 표준 JSON 구조 패턴 유형을 수용하도록 설계되었습니다. 표준 패턴의 예는 다음과 같습니다.`{“data”:[{“key”:“value”}, {“key”:“value”},...]}`
...
분리 확인란(기본값:unchecked)는 JSON을 키/값 맵으로 병합할지 여부를 나타내기 위해 제공됩니다.

* 이 **확인란을 선택 해제하면** 샘플 JSON을 구문 분석하여 배열 개체를 찾습니다. 사용자는 Adobe Campaign에서 사용자가 사용하려는 배열을 정확하게 파악할 수 있도록 트리밍된 버전의 API 응답 샘플 JSON 형식을 제공해야 합니다. 워크플로우 작성 시 중첩된 배열 개체의 경로가 결정 및 기록되므로 실행 시 API 호출에서 수신한 JSON 응답 본문에서 해당 배열 개체에 액세스하는 데 사용할 수 있습니다.

* 확인란을 활성화 **** (선택)하면 샘플 JSON이 분리되고 제공된 샘플 JSON에 지정된 모든 속성이 출력 임시 테이블의 열을 만드는 데 사용되고 열 정의 탭에 표시됩니다. 샘플 JSON에 배열 개체가 있으면 해당 배열 개체의 모든 요소도 분리됩니다.


구문 **분석이 유효하다면**&#x200B;메시지가 나타나고 &quot;열 정의&quot; 탭에서 데이터 매핑을 사용자 정의하도록 초대합니다. 다른 경우 오류 메시지가 표시됩니다.
