...
>[!IMPORTANT]
>
>Adobe 워크플로우 임계값인 20에 도달하지 않더라도 다음 작업을 수행하는 것이 좋습니다. **시간에 따라 워크플로우 실행 분산**. 워크플로우 실행을 지연시키면 인스턴스의 성능이 향상됩니다.

워크플로우를 시작하기 전에 [!DNL Campaign Standard] 워크플로우를 실행할 시스템 실제 메모리가 충분한지 확인합니다. 사용 가능한 메모리가 부족한 경우 서버의 로드가 줄고 시스템 메모리가 늘어날 때까지 워크플로우 실행이 지연된다는 메시지가 표시됩니다.
...
워크플로우가 제대로 종료되도록 하려면 를 사용하여 워크플로우의 마지막 전환을 그대로 두지 마십시오 **[!UICONTROL End activity]**.
...
워크플로우를 호출할 때 정의된 매개 변수의 이름 및 개수가 동일한지 확인합니다( 참조) [이 페이지](../../automating/using/defining-parameters-calling-workflow.md). 매개 변수의 유형은 예상 값과 일치해야 합니다.
