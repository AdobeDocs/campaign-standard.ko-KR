---
title: 컨트롤 그룹 만들기
description: 이 사용 사례에서는 제어 그룹을 만드는 방법을 보여 줍니다.
...
# 컨트롤 그룹 만들기 {#building-control-group}
...
* 지정된 [모집단](../../automating/using/query.md) 타깃팅할 쿼리 활동.
* 이 [모집단에서 임의](../../automating/using/segmentation.md) 제어 그룹을 격리하는 세그멘테이션 활동입니다.
* 메시지를 기본 [타겟으로 보내는 이메일 배달](../../automating/using/email-delivery.md) 활동.
* 대상에서 제외된 프로필(임의 제어 그룹)을 업데이트하기 위해 데이터 [](../../automating/using/update-data.md) 활동을 업데이트합니다.

![](assets/wkf_control-group.png)

## 프로필 리소스 확장 {#extending-profile}

첫째, 제어 그룹에 해당하는 새 필드로 **[!UICONTROL Profile]** 리소스를 확장해야 합니다. 워크플로우가 실행되면 이 필드에서 대상에서 제외된 프로파일을 확인합니다.

1. > **[!UICONTROL Administration]** > **[!UICONTROL Development]** **[!UICONTROL Custom Resources]**&#x200B;에서 을 **[!UICONTROL Create]**&#x200B;클릭합니다.
1. 아직 연장하지 않은 경우 리소스 **[!UICONTROL Extend an existing resource]** 를 선택하고 **[!UICONTROL Profile]** 선택합니다.
1. 탭 **[!UICONTROL Data structure]** 에서 제어 그룹에 대한 새 필드를 추가하고 필드 **[!UICONTROL Boolean]** 에 대해 **[!UICONTROL Type]** 선택합니다.

   ![](assets/wkf_control-group-profile-field.png)

1. 탭에서 섹션 **[!UICONTROL Screen definition]** 을 **[!UICONTROL Detail screen configuration]** 연 다음 각 프로필에 대해 표시되도록 방금 만든 필드를 선택합니다.
...
1. 탭에서 **[!UICONTROL Limitation]** 옵션을 **[!UICONTROL Random sampling]** 선택했는지 확인합니다.

   ![](assets/wkf_control-segment-limitation.png)

1. 초기 모집단(예: 10%)의 백분율을 정의하고 클릭합니다 **[!UICONTROL Confirm]**. 제어 그룹은 임의로 선택된 대상 모집단으로부터 10%로 구성됩니다.
...
1. 탭에서 리소스 **[!UICONTROL Fields to update]** 에 추가한 제어 그룹 필드를 **[!UICONTROL Profile]** 조건으로 **[!UICONTROL Destination]** 선택하고 true를 조건으로 입력합니다.
...
워크플로우 **[!UICONTROL Start]** 를 실행하려면 을 클릭합니다.
...
이제 메시지를 받은 사람이 메시지에서 제외되고 메시지를 받지 못한 작은 그룹과 비교하여 반응하는 방식을 비교할 수 있습니다.

## 동일한 제어 그룹 재사용 {#reusing-same-control-group}

위의 예에서는 글로벌 제어 그룹을 만들 수 있습니다. 글로벌 제어 그룹은 게재와 독립적으로 프로필 속성으로 저장됩니다. 실제로, 리소스 확장 기능의 일부로 만들어진 새 &quot; **[!UICONTROL Profile]** 제어 그룹&quot; 필드는 위의 작업 과정이 실행된 후 업데이트됩니다.
...
1. 창에서 **[!UICONTROL Add a rule - Profiles (attributes)]** &quot;제어 그룹&quot;(리소스에 추가한 필드)을 선택하고 필터 조건 **[!UICONTROL Profile]** **[!UICONTROL Yes]** 으로 선택합니다.
