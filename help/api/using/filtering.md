...
각 리소스에 대해 필터를 사용할 수 있습니다. 리소스와 연결된 필터를 식별하려면 리소스 메타데이터에 대해 GET 요청을 수행해야 합니다. 이 요청은 지정된 리소스에 대해 모든 필터가 정의된 URL을 반환합니다. 메타데이터에 대한 자세한 정보는 [이 섹션](../../api/using/metadata-mechanism.md)을 참조하십시오.
...
* **data** 필드는 필터를 사용하는 방법에 대한 샘플을 제공합니다.
* **메타데이터** 노드는 필터 매개 변수를 설명합니다.
* **조건** 노드는 필터가 수행할 작업을 설명합니다. 메타데이터 노드에 설명된 필터 매개 변수는 필터 조건을 만드는 데 사용됩니다. 각 필터 조건에 대해 **enabledIf**&#x200B;가 true이면 **expr**&#x200B;이 적용됩니다.
