...
* 이제 [동적 보고서]의 게재 성과 계산에서 교정 및 트랩 프로필을 제외합니다. (CAMP-47338)
...
* 동적 보고서에서 **교정쇄 제외** 세그먼트가 제거되었습니다. (CAMP-46161)
...
* 배치 프로세스 로그 테이블(**xtkjoblog**)에 대한 만료 옵션이 적용되지 않는 문제를 해결했습니다. 이 버그로 인해 테이블이 올바르게 삭제되지 않는 문제가 있었습니다.
...
* 내부 링크를 클릭한 후 탐색 문제가 발생하는 문제를 해결했습니다(예: 교정쇄 요약 화면에서 상위 게재에 액세스할 때).
...
* 교정쇄를 처리하는 데 더 오래 걸릴 수 있는 문제를 수정했습니다. (CAMP-45048)
