...
* 푸시 알림 채널을 구성할 때 iOS용 푸시 공급자 플랫폼은 다음과 같아야 합니다 **apns** Android의 경우 및 **gcm**.
...
* 의 문제가 해결되었습니다. **[!UICONTROL Reconciliation]** 활동.
...
* 이제 라우팅 외부 계정을 비활성화하면 영향을 받는 게재(이메일, SMS 및 푸시) 및 **미리 보기** 이러한 게재에서는 단추가 숨겨집니다.
...
* 다음 **핫 클릭** 보고서가 개선되었습니다. 또한 게재에 정의된 각 조건부 콘텐츠에 따라 핫 클릭을 표시하고 반복 게재 또는 트랜잭션 메시지의 각 실행에 대한 핫 클릭을 표시할 수 있습니다. 자세한 내용은 [세부 설명서](../../sending/using/receiving-alerts-when-failures-happen.md#creating-a-delivery-alerting-criterion)를 참조하세요.
...
* 사용자가 목록을 내보낼 수 있는 문제를 해결했습니다. 이제 을(를) 가진 사용자만 **[!UICONTROL Export]** 역할에게 허용됩니다.

_이메일, SMS 메시지 및 DM_

* 의 문제가 해결되었습니다. **updateDeliveryExecInfo** 을 설정하는 워크플로 **게재하기** SMS 게재에 대해 0으로 표시기.
* 다음에서 **고급 매개 변수** 게재 템플릿의 속성 중 **라우팅** 이제 드롭다운 목록에 템플릿 메시지 유형에 해당하는 외부 계정만 표시됩니다. 예를 들어 이메일 게재 템플릿에는 이메일 외부 계정만 표시됩니다.
* 의 문제가 해결되었습니다. **[!UICONTROL Text]** 테스트 프로필에 대해 정의된 기본 이메일 형식입니다.
* 게재의 일정 정의 화면에서 기본 시간대를 선택할 때에서 Javascript 오류가 발생하는 문제를 수정했습니다.
* 전송 로그에 트랩이 표시되지 않는 문제를 해결했습니다.
* 게재 만들기 마법사의 템플릿 선택 화면에서 후속 및 A/B 테스트 템플릿은 이제 기본적으로 숨겨집니다. 자세한 내용은 [자세한 설명서](../../channels/using/creating-an-email.md).
* 모든 사용자가 게재를 보낼 수 있는 문제를 해결했습니다. 이제 을(를) 가진 사용자만 **[!UICONTROL Start deliveries]** 역할에게 허용됩니다. 자세한 내용은 [자세한 설명서](../../sending/using/confirming-the-send.md).

_푸시 알림_

* 의 문제가 해결되었습니다. **캠페인 추적 끝점** 보고할 수 없는 URL입니다.
...
* 상단 막대에서 **[!UICONTROL Timeline]** 링크가 제거되고 다음에 대한 링크로 대체되었습니다. **[!UICONTROL Programs & Campaigns]** .
...
* 다음 **[!UICONTROL Bounce]** 지표가 개선되어 더 정확한 결과를 얻을 수 있습니다.
...
* 다음을 최적화했습니다. **[!UICONTROL by email]** 랜딩 페이지에서 새 구독자를 데이터베이스 프로필과 조정하는 데 사용되는 필터입니다.
...
<table> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td> 새 워크플로우를 사용하여 다른 워크플로우에서 워크플로우 트리거 <span class="uicontrol">신호</span> 활동.<br /> 한 워크플로우를 다른 워크플로우에서 시작할 수 있으므로 보다 복잡한 고객 여정을 지원할 수 있습니다. 고객 여정을 보다 잘 모니터링하고 문제가 있을 경우 대응할 수 있습니다.<br /> 몇 가지 워크플로우 활동이 업데이트되었습니다.<br /> 
    <ul> 
...
* 의 원인이 되는 문제가 해결되었습니다. **[!UICONTROL Request confirmation before sending messages]** 옵션을 선택 취소하고 게재를 저장한 후에도 선택된 상태로 유지하는 옵션입니다.
...
<table> 
 ...
 <tbody> 
  <tr> 
   ...
   <td> 게재 및 추적 로그와 같은 로그를 내보내 원하는 보고 또는 BI 도구에서 처리합니다. 증분 쿼리와 함께 간단한 워크플로우를 사용하여 새 로그의 정기적인 내보내기를 자동화할 수 있습니다.<br /> 리소스 선택기에서 사용할 수 있는 로그 리소스 외에도 <a href="../../automating/using/incremental-query.md">증분 쿼리</a> 및 <a href="../../automating/using/extract-file.md">파일 추출</a> 활동:<br /> 
    <ul> 
     <li> <span class="uicontrol">증분 쿼리</span> 이제 날짜 필드를 사용하여 새 데이터 또는 업데이트된 데이터를 검색할 수 있습니다. 이전에는 마지막 실행 이후 업데이트된 경우에도 이전 실행의 모든 결과가 자동으로 제외되었습니다. </li> 
     <li> <span class="uicontrol">파일 추출</span> 이제 ID 대신 열거형 값에 대한 레이블을 내보낼 수 있습니다. </li> 
    </ul> 이러한 활동은 모든 지역 및 조직 단위에 액세스할 수 있는 관리자가 사용할 수 있습니다.<br /> 로그 내보내기에 대한 자세한 내용은 <a href="../../automating/using/exporting-logs.md">자세한 설명서</a>.<br /> </td> 
  </tr> 
  <tr> 
...
* 다음 **[!UICONTROL Access authorization]** 옵션이 랜딩 페이지 속성으로 돌아왔습니다.
...
* 다음과 같은 경우 사용자가 세그먼트의 모집단을 제한할 수 없는 문제를 해결했습니다 **[!UICONTROL Temporary resource]** 이(가)에서 선택되었습니다. **[!UICONTROL General]** 탭 / **[!UICONTROL Segmentation]** .
* **[!UICONTROL Scheduler]** 활동은 더 이상 10분마다 두 번 이상 워크플로우를 트리거하도록 설정할 수 없습니다.
* 을(를) 방지할 수 있는 문제를 해결했습니다. **[!UICONTROL Use common columns]** 에서 제대로 작업 **[!UICONTROL Union]** 활동.
...
* 의 원인이 될 수 있는 문제를 해결했습니다. **[!UICONTROL Save audience]** 옵션이 실패할 경우 워크플로우의 활동이 실패함 **[!UICONTROL Share in Adobe Marketing Cloud]** 이(가) 확인되었습니다.
