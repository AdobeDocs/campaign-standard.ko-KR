...
* 새 옵션을 사용하면 워크플로우를 시작하기 전에 Campaign Standard에서 **시스템에서 실제 메모리** 가용성을 확인할 수 있습니다. 메모리 양이 너무 낮으면 시스템 메모리가 이 임계값에 도달할 때까지 워크플로 실행이 지연됩니다. 이로 인해 성능 저하를 방지하고 서비스 중단 위험을 줄일 수 있습니다. 서버에 대한 로드가 줄고 메모리가 증가하면 워크플로우가 자동으로 다시 시작됩니다. 이 옵션은 읽기 전용이므로 수정할 수 없습니다.
