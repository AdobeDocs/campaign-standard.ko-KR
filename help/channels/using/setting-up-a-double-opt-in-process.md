...
1. 가입자에게 이제 등록되었음을 알리도록 확인 페이지의 컨텐츠를 조정합니다.
...
즉시 사용 가능한 규칙을 복제하여 특정 [유형 규칙을](../../administration/using/about-typology-rules.md)만들어야 합니다. 이 규칙을 사용하면 아직 계약을 확인하지 않은 채 블랙리스트에 추가된 프로필로 메시지를 보낼 수 있습니다. 기본적으로 유형 지정 규칙은 옵트아웃(즉, 블랙리스트에 추가된) 프로필을 제외합니다. 이 유형 규칙을 만들려면 다음 단계를 수행하십시오.
