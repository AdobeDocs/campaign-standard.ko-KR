...
### 푸시 채널에 대해 자세히 알아보려면 유용한 리소스 추천은 무엇입니까?{#resource-push}
...
### Campaign에서 푸시 토큰을 가져오려면 어떻게 해야 합니까?{#push-token-acquisition}

프로비저닝 팀이 Adobe Campaign Standard에서 푸시 채널 프로비전을 완료했는지 확인합니다. SDK에서 setPushIdentifier API를 구현합니다. 자세한 정보는 이 [페이지](https://aep-sdks.gitbook.io/docs/using-mobile-extensions/adobe-campaign-standard#set-up-push-messaging)를 참조하십시오.

### Campaign에 푸시 토큰 및 ECID가 있는 경우 푸시 알림을 전송하려면 무엇이 필요합니까?{#sending-push}

푸시 알림을 전송하려면 유효한 푸시 인증서를 .pem 형식으로 제공해야 합니다. 이 인증서에 대한 암호가 필요하지 않습니다.

### .pem 인증서 대신 .p12 인증서가 있으면 어떻게 합니까?{#certificates}

터미널에서 아래 명령을 실행하여 .p12 인증서를 .pem 인증서로 변환할 수 있습니다. 전환 지침에 사용할 수 있는 여러 온라인 리소스도 있습니다.
...
### iOS App(Android용 N/A)에 대해 제작 인증서와 샌드박스 인증서를 동시에 업로드할 수 있습니까?{#prod-sandbox-certificate}

아니요. 앱이 샌드박스 또는 프로덕션 모드에서 작동하며 설정되면 다른(즉, 샌드박스에서 프로덕션 앱으로) 변경할 수 없습니다. 먼저 샌드박스 모드에서 앱을 테스트한 다음 프로덕션 모드로 전환하는 것이 좋습니다.

제작 모드로 변경하려면 다른 앱을 만들어야 합니다. 샌드박스 확인란을 선택하지 않고 프로덕션 인증서를 업로드해야 합니다.
...
### 푸시 알림을 pushtry.com에서 성공적으로 보낼 수는 있지만 Campaign을 통해서는 보낼 수 없습니다.{#push-not-sending}

[여기](../../administration/using/push-payload.md)에 제공된 푸시 페이로드 지침을 따르고 있는지 확인하십시오.

Android의 경우 캠페인은 알림 페이로드가 아닌 데이터 페이로드만 지원합니다

### Adobe Campaign Standard의 관리 섹션에 앱을 구성했지만 모바일 앱은 전달 속성에서 사용할 수 없습니다.{#mobile-app-unavailable}

앱은 전달 속성에서 사용할 수 있기 전에 유효한 푸시 인증서가 업로드되어야 합니다.

### 이 페이지의 모든 지침을 시도했지만 Campaign에서 푸시를 보낼 수 없습니다.{#push-troubleshoot}

고객 관리 티켓을 열어 주십시오.

### 푸시 알림이 캠페인에서 배달되지만 미디어 파일이 표시되지 않습니다.{#media-file-unavailable}

모바일 앱 개발자는 앱에서 미디어 파일에 대한 지원을 처리해야 합니다. 네트워크 대역폭이 미디어 파일이 렌더링되지 않는 경우도 있습니다. 추가 포인터를 보려면 이 [page](../../administration/using/image-push-notification.md)을 참조하십시오.

### Campaign에서 푸시 보고를 활성화하려면 어떻게 해야 합니까?{#push-reporting-enable}

아래의 단계를 수행하십시오.

* 푸시 추적 포스트백을 구성합니다. 지침은 [여기](../../administration/using/configuring-a-mobile-application.md)에 있습니다.
* Mobile Core에서 trackAction API를 구현합니다. 자세한 내용은 이 [페이지](https://aep-sdks.gitbook.io/docs/using-mobile-extensions/mobile-core/mobile-core-api-reference)를 참조하십시오.

자세한 지침은 이 [페이지](../../administration/using/push-tracking.md)에서 확인할 수 있습니다.

### 푸시 채널에 사용할 수 있는 보고서는 무엇입니까?{#push-report-available}

기본 보고서는 푸시 채널용 Adobe Campaign에서 사용할 수 있습니다. 이 [설명서](../../reporting/using/push-notification-report.md)를 참조하십시오.

각 푸시 지표가 계산되는 방식을 이해하려면 이 [페이지](../../reporting/using/indicator-calculation.md#push-notification-delivery)을 참조하십시오.

### 푸시 및 인앱 메시지에 링크가 지원됩니까?{#deeplink-push}

예, 링크는 푸시 메시지에서 지원됩니다. 링크는 다음을 포함해야 합니다.

* 설명이 작동하기 위해 배달 추적을 비활성화해야 한다는 것을 나타내는 언어입니다.
