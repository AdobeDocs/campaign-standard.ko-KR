...
<!--(the cart abandonment example as explained in [this section](../../channels/using/getting-started-with-transactional-msg.md#transactional-messaging-operating-principle))--> 이벤트를 만들고 게시하면 해당 트랜잭션 메시지가 자동으로 만들어집니다.
...
메시지가 준비되면 테스트하고 게시할 수 있습니다. [트랜잭션 메시지 테스트](../../channels/using/testing-transactional-message.md) 및 [트랜잭션 메시지 주기](../../channels/using/publishing-transactional-message.md)를 참조하십시오.
...
1. **[!UICONTROL Marketing plans]** > **[!UICONTROL Transactional messages]** > **[!UICONTROL Transactional messages]**&#x200B;을(를) 선택합니다.
...
해당 이벤트 구성 화면의 왼쪽 영역에 있는 링크를 통해 트랜잭션 메시지에 직접 액세스할 수도 있습니다. [이벤트 미리 보기 및 게시](../../channels/using/publishing-transactional-event.md#previewing-and-publishing-the-event)를 참조하세요.
...
>[!NOTE]
>
>이 섹션에서는 **이벤트 기반** 트랜잭션 메시지를 편집하는 방법에 대해 설명합니다. **프로필 기반** 트랜잭션 메시지 특성에 대한 자세한 정보는 [아래](#profile-transactional-message-specificities)를 참조하세요.
>
...
예를 들어 장바구니에 제품을 추가한 웹 사이트 사용자에게 알림을 보내고 구매를 완료하지 않고 사이트를 나가는 경우가 있습니다. 이 예제는 [트랜잭션 메시지 작동 원칙](../../channels/using/getting-started-with-transactional-msg.md#transactional-messaging-operating-principle) 섹션에 나와 있습니다.
...
고객 마케팅 프로필을 기반으로 트랜잭션 메시지를 보낼 수 있습니다. 이렇게 하면 모든 프로필 정보를 활용하여 메시지 콘텐츠를 개인화하고, 구독 취소 링크를 사용하고, [피로도 규칙](../../sending/using/fatigue-rules.md)과 같은 마케팅 유형화 규칙을 적용할 수 있습니다.

* 이벤트 기반 트랜잭션 메시지와 프로필 기반 트랜잭션 메시지의 차이점에 대한 자세한 내용은 [이 섹션](../../channels/using/getting-started-with-transactional-msg.md#transactional-message-types)을 참조하세요.
...
1. 편집할 트랜잭션 메시지를 [이동](#accessing-transactional-messages)합니다.
1. 트랜잭션 메시지에서 **[!UICONTROL Content]** 섹션을 클릭합니다. 트랜잭션 전자 메일 템플릿 외에 **[!UICONTROL Profile]** 리소스를 타겟팅하는 전자 메일 템플릿을 선택할 수도 있습니다.
...
1. 기본 이메일 템플릿을 선택합니다. 모든 마케팅 이메일과 마찬가지로 **구독 취소 링크**&#x200B;가 포함되어 있습니다.
   ...
   템플릿에 대한 자세한 내용은 [이 섹션](../../designing/using/using-reusable-content.md#content-templates)을 참조하세요.

1. 또한 실시간 이벤트를 기반으로 하는 구성과 달리, 메시지를 개인화할 수 있도록 **모든 프로필 정보에 직접 액세스**&#x200B;할 수 있습니다. 다른 표준 마케팅 이메일과 마찬가지로 [개인화 필드](../../designing/using/personalization.md#inserting-a-personalization-field)를 추가할 수 있습니다.
