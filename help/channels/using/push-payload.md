...
페이로드에는 사용자 지정 키/값 쌍을 비롯한 모든 푸시 알림 배달 콘텐츠를 포함하는 데이터 메시지가 포함되어 있으며, 클라이언트 앱은 필요한 경우 푸시 알림을 빌드하고 표시하는 메시지를 처리하거나 다른 비즈니스 논리를 추가해야 합니다.

Android 페이로드의 측면을 이해하려면 메시징 개념 [및 옵션(fcm)](https://firebase.google.com/docs/cloud-messaging/concept-options)을 참조하십시오.

>[!NOTE]
>
>Android 페이로드의 알림 메시지에 대한 지원이 2018년 1월 현재 삭제되어 사용자가 앱과 상호 작용할 필요 없이 앱의 절전 모드를 해제할 수 있습니다.

### 캠페인 표준 구성과 페이로드 속성 간의 매핑 {#mapping-payload}

| 캠페인 구성 | iOS에서 영향을 받은 속성 | Android의 영향을 받은 속성 | 설명 |
|:-:|:-:|:-:|:-:|
| 메시지 제목 <br>메시지 본문 | 알림→제목 <br> 알림 요청 | 제목 <br>본문 | 이 데이터에는 경고 메시지에 대한 세부 사항이 포함됩니다.<br>제목 및 본문 키는 경고의 내용을 제공합니다. |
| 사운드 재생 | 사운드 | 사운드 | 경고와 함께 재생할 사용자 정의 사운드입니다. |
| 배지의 값 | 배지 | 배지 | 앱의 아이콘을 배지하는 데 사용할 정수 값입니다. |
| 기본값 추가 | uri | 북미 | 시각화를 사용하면 사용자를 웹 브라우저 페이지를 여는 대신 애플리케이션 내에 있는 컨텐츠로 직접 가져올 수 있습니다. |
| 카테고리 | category | category | 원격 알림과 함께 사용자 지정 작업을 표시합니다. <br>카테고리 키는 시스템이 경고 인터페이스에서 해당 카테고리에 대한 작업을 단추로 표시하는 데 도움이 됩니다. |
| 사용자 정의 필드 | custom_field1, custom_field2... | custom_field1, custom_field2... | 앱에 전송하려는 모든 사용자 지정 데이터 |
| 리치 미디어 컨텐츠 URL(이미지, gif, 오디오 및 비디오 파일)<br>(iOS 10 이상에만 적용 가능) | media-attachment-url | 북미 | 알림에 리치 컨텐츠를 추가할 미디어 파일의 URL입니다. <br>이 URL에 대한 값을 제공하면 mutable-content 플래그가 페이로드로 자동 전송됩니다. <br> (iOS 10 이상에만 적용) |
| 가변 컨텐츠 <br> (iOS 10 이상에만 적용) | mutable-content | 북미 | 앱의 알림 서비스 익스텐션은 mutable-content 키로 모든 원격 알림을 &quot;차단&quot;하며, 요청 페이로드의 컨텐츠를 처리/조작할 수 있도록 허용하며, 이를 통해 알림을 사용자 지정하는 데 사용할 수 있습니다. 이 기능의 사용 사례로는 여러 미디어 다운로드 및 표시, 푸시 페이로드에 있는 암호화된 데이터의 암호 해독 등이 있습니다. 자세한 내용은 원격 [알림의 페이로드 수정에서 확인할 수 있습니다](https://developer.apple.com/library/archive/documentation/NetworkingInternet/Conceptual/RemoteNotificationsPG/ModifyingNotifications.html). <br>(iOS 10 이상에만 적용) |
| 사용 가능한 컨텐츠 | 컨텐츠 사용 가능 | 북미 | 이 옵션을 선택하면 iOS 앱이 백그라운드/일시 중단 상태일 때 깨어날 수 있습니다. 깨어나는 것은 앱이 백그라운드에서 실행되며 푸시 알림 데이터 페이로드를 수신하는 적절한 이벤트 핸들러는 제어권을 가지며, 사용자 지정 푸시 알림을 빌드하고 동일하게 표시하는 것을 포괄적으로 포함하여 모든 계산을 위해 데이터를 사용할 수 있음을 의미합니다. 알림 배달이 [](https://developer.apple.com/library/archive/documentation/NetworkingInternet/Conceptual/RemoteNotificationsPG/CreatingtheNotificationPayload.html)있는 Wake up App에서 자세한 내용을 확인할 수 있습니다. |
| 리치 미디어 컨텐츠 URL(이미지 파일)<br>(Android에만 적용 가능) | 북미 | media-attachment-url | 알림에 리치 컨텐츠를 추가할 이미지 파일의 URL입니다. |
| 북미 | _mId<br>_dId | _mId <br>_dId | broadlogId 및 deliveryId의 값입니다.<br>이러한 속성은 앱이 푸시 알림을 클릭/연 시간을 추적하기 위해 추적 포스트백을 호출하려는 경우에 필요합니다. 이 정보는 사용자의 개입 없이 앱 서버에서 내부적으로 계산되고 전송됩니다.<br>포스트백에 대한 정보는 이 [페이지에서](https://helpx.adobe.com/campaign/kb/config-app-in-launch.html#PIIpostback)찾을 수 있습니다. |
