...
Adobe Campaign을 사용하여 **모든 사용자에 대한 익명 트랜잭션 푸시 알림** 모바일 애플리케이션으로부터 알림을 받도록 선택한 사용자.

이 경우, **이벤트 자체에 포함된 데이터를 게재 타겟 정의에 사용합니다**. Adobe Campaign 통합 프로필 데이터베이스의 데이터가 사용되지 않습니다.
...
>[!NOTE]
>
>를 사용하여 이벤트 기반 트랜잭션 푸시 알림의 콘텐츠를 개인화할 수 있습니다 [이벤트 속성](../../channels/using/configuring-transactional-event.md#defining-the-event-attributes) (이벤트의 데이터) 및 [이벤트 강화](../../channels/using/configuring-transactional-event.md#enriching-the-transactional-message-content) (Campaign 데이터베이스의 데이터). 자세한 내용은 [아래 예](#sending-event-based-transactional-push-notification).

이벤트는 다음 세 가지 요소를 포함해야 합니다.

* A **등록 토큰**: 하나의 모바일 애플리케이션 및 하나의 장치에 대한 사용자 ID입니다. Adobe Campaign 데이터베이스의 프로필에 해당하지 않을 수 있습니다.
* A **모바일 애플리케이션 이름** (모든 장치용 하나 - Android 및 iOS). 사용자의 장치에서 푸시 알림을 받는 데 사용할 Adobe Campaign에 구성된 모바일 애플리케이션의 ID입니다. 자세한 내용은 [모바일 애플리케이션 구성](../../administration/using/configuring-a-mobile-application.md).
* A **푸시 플랫폼** ( Android의 경우 &quot;gcm&quot;, iOS의 경우 &quot;apns&quot;).

이벤트를 구성하려면 아래 단계를 수행하십시오.

1. 이벤트 구성을 만들 때 **[!UICONTROL Push notification]** 채널 및 **[!UICONTROL Real-time event]** 타겟팅 차원(참조) [이벤트 만들기](../../channels/using/configuring-transactional-event.md#creating-an-event)).
...
1. 메시지 콘텐츠를 보강할 수도 있습니다. 이렇게 하려면 이벤트 구성에 연결된 테이블에서 필드를 추가합니다( 참조) [이벤트 강화](../../channels/using/configuring-transactional-event.md#enriching-the-transactional-message-content)).
...
1. [이벤트 미리 보기 및 게시](../../channels/using/publishing-transactional-event.md#previewing-and-publishing-the-event).
   ...
   이벤트가 게시되면 새 이벤트에 연결된 트랜잭션 푸시 알림이 자동으로 만들어집니다. 이제 방금 만들어진 메시지를 수정하고 게시할 수 있습니다( 참조). [이 섹션](#sending-event-based-transactional-push-notification)).
...
1. 만든 트랜잭션 메시지로 이동하여 편집합니다. 자세한 내용은 [트랜잭션 메시지 액세스](../../channels/using/editing-transactional-message.md#accessing-transactional-messages).

   ![](assets/message-center_push_message.png)

1. 을(를) 클릭합니다. **[!UICONTROL Content]** 메시지 제목과 본문을 수정하려면 차단하십시오.
...
1. 개인화 필드를 삽입하여 이벤트를 만들 때 정의한 요소를 추가할 수 있습니다( 참조) [이벤트 속성 정의](../../channels/using/configuring-transactional-event.md#defining-the-event-attributes)).
   ...
   이러한 필드를 찾으려면 항목 옆에 있는 연필을 클릭하고 **[!UICONTROL Insert personalization field]** 을(를) 선택합니다. **[!UICONTROL Context]** > **[!UICONTROL Real-time event]** > **[!UICONTROL Event context]**.

   ![](assets/message-center_push_personalization.png)

   푸시 알림 콘텐츠 편집에 대한 자세한 내용은 [푸시 알림 준비 및 보내기](../../channels/using/preparing-and-sending-a-push-notification.md).

1. Adobe Campaign 데이터베이스의 추가 정보를 사용하려는 경우에도 트랜잭션 메시지 콘텐츠를 보강할 수 있습니다( 참조) [이벤트 강화](../../channels/using/configuring-transactional-event.md#enriching-the-transactional-message-content)).
...
*&quot;안녕하세요, 제인 그린, 탑승이 막 시작되었어요! B18게이트로 가십시오.&quot;*

## 프로필을 타겟팅하는 트랜잭션 푸시 알림 {#transactional-push-notifications-targeting-a-profile}

트랜잭션 푸시 알림을 보낼 수 있습니다 **모바일 애플리케이션을 구독한 Adobe Campaign 프로필에**. 이 게재에는 다음이 포함될 수 있습니다 [개인화 필드](../../designing/using/personalization.md#inserting-a-personalization-field)수신자 이름과 같이 Adobe Campaign 데이터베이스에서 직접 검색합니다.

이 경우 이벤트에 일부 필드가 포함되어야 합니다 **Adobe Campaign 데이터베이스의 프로필과 조정 허용**.
...
프로필이 구독한 모바일 애플리케이션은 **[!UICONTROL Mobile App Subscriptions]** 이 프로필의 탭입니다. 이 탭에 액세스하려면 프로필을 선택하고 **[!UICONTROL Edit profile properties]** 오른쪽에 있는 단추입니다.
...
1. 이벤트 구성을 만들 때 **[!UICONTROL Push notification]** 채널 및 **[!UICONTROL Profile]** 타겟팅 차원(참조) [이벤트 만들기](../../channels/using/configuring-transactional-event.md#creating-an-event)).
...
1. 트랜잭션 메시지를 개인화하려는 경우 이벤트에 필드를 추가합니다( [이벤트 속성 정의](../../channels/using/configuring-transactional-event.md#defining-the-event-attributes)).

   >[!NOTE]
   >
   >데이터 보강 작업을 수행하려면 필드를 하나 이상 추가해야 합니다. 다음과 같은 다른 필드를 만들 필요는 없습니다 **이름** 및 **성** Adobe Campaign 데이터베이스의 개인화 필드를 사용할 수 있으므로

1. 이벤트를 **[!UICONTROL Profile]** 리소스( [이벤트 강화](../../channels/using/configuring-transactional-event.md#enriching-the-transactional-message-content))을 선택하고 이 데이터 보강 을(를) (으)로 선택합니다 **[!UICONTROL Targeting enrichment]**.
...
1. [이벤트 미리 보기 및 게시](../../channels/using/publishing-transactional-event.md#previewing-and-publishing-the-event).
   ...
   이벤트가 게시되면 새 이벤트에 연결된 트랜잭션 푸시 알림이 자동으로 만들어집니다. 이제 방금 만들어진 메시지를 수정하고 게시할 수 있습니다( 참조). [이 섹션](#sending-profile-based-transactional-push-notification)).
...
트랜잭션 푸시 알림은 특정 [대상 매핑](../../administration/using/target-mappings-in-campaign.md) 여기에는 이 유형의 게재를 전송하는 데 필요한 기술 설정이 포함되어 있습니다.
...
1. 를 확장합니다. **[!UICONTROL Advanced parameters]** 섹션을 참조하십시오.
