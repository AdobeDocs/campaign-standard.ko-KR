...
사용자는 워크플로 엔진에서 나오는 세그먼트 코드로 로그를 확장할 수 있습니다.

세그먼트 코드는 워크플로에 정의되어야 합니다.
...
이제 **[!UICONTROL ACS ID]** 필드를 사용할 수 있으며 새 요소를 수동으로 생성하거나 API 또는 가져오기 워크플로에서 삽입되면 자동으로 채워집니다. ACS ID 필드는 UUID 필드이며 인덱싱됩니다.
